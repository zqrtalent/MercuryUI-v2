...
public:
	...
	int									m_nActAmount;		// Action amount value.
	int									m_nPotAmount;		// Pot amount value.
	int									m_nTotalPotAmount;	// Total Pot amount value.
	int									m_nBalanceAmount;	// Balance amount.
...
public:
	...
	int										m_nRoundPotAmount;		// Round pot amount.
...
public:
	...
	void			StartRound				(PokerRoundType roundType, EnumerableObject<PokerHandCards>* pListHandCards, EnumerableObject<PokerPotAmountDetail>* pListPots);
	void			FinishRound				(int nTotalPotAmount, int nPotAmount, int nRakeAmount);
	void			FinishRound2			();
	bool			UpdateCurrentRound		(int nTotalPotAmount, int nPotAmount, int nRakeAmount);

	void			Action					(int nSeat, PokerActionTypes actionType, int nAmount, int nBalance, int nPotAmount, int nTotalPotAmount, int nTimeSec, 
											EnumerableObject<PokerHandCards>* pCardsShown = NULL);

	bool			ChangeLastActionAmounts	(int nSeat, int nAmountNew, int nBalanceNew, int nPotAmountNew, int nTotalPotAmountNew);

	void			ActionWithHandCard		(int nSeat, PokerActionTypes actionType, int nAmount, int nBalance, int nPotAmount, int nTotalPotAmount, int nTimeSec, PokerHandCards* pHandCards);
	void			ClearHistory			();
