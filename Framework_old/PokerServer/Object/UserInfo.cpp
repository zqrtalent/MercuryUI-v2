...
BEGIN_DECLARE_VAR(UserInfo, Serializable)
	...
	DECLARE_VAR(_T("BalanceAmount"),			_T(""),		VariableType::VT_Int32, offsetof(UserInfo,m_nBalanceAmount), false, false, false)
	DECLARE_VAR(_T("InPlayAmount"),				_T(""),		VariableType::VT_Int32, offsetof(UserInfo,m_nInPlayAmount), false, false, false)
	DECLARE_VAR(_T("PlayBalance"),				_T(""),		VariableType::VT_Int32, offsetof(UserInfo,m_nPlayBalance), false, false, false)
	DECLARE_VAR(_T("InPlayPlayBalance"),		_T(""),		VariableType::VT_Int32, offsetof(UserInfo,m_nInPlayPlayBalance), false, false, false)	
...
UserInfo::SetAvatarInfo(PokerAvatar* pAvatarInfo, bool bCopy){
	...
	if( bCopy ){
		...
		if( pAvatarInfo->m_binImage.GetBinarySize() > 0 ){
			void* lpData = pAvatarInfo->m_binImage.LockMemory();
			m_pAvatarInfo->m_binImage.SetBinarySize(pAvatarInfo->m_binImage.GetBinarySize(), lpData);
...
UserInfo::UpdateAvatarInfo(int nAvatarId, void* lpAvatarData, UINT nDataSize){
	...
	return (m_pAvatarInfo->m_binImage.SetBinarySize(nDataSize, lpAvatarData) == TRUE);
	}