...
RETURNS:		std::string : the hexadecimal MD5 checksum for the specified file
ARGUMENTS:		std::string& strFilePath : the full pathname of the specified file
...
*****************************************************************************************/
/*
std::string CMD5Checksum::GetMD5(CFile& File)
{
	try
	{
...
void CMD5Checksum::ByteToDWord(DWORD* Output, BYTE* Input, UINT nLength)
{
	//entry invariants
	ASSERT( nLength % 4 == 0 );
	ASSERT( !IsBadReadPtr(Output, nLength/4) );
	ASSERT( !IsBadReadPtr(Input, nLength) );
...
void CMD5Checksum::DWordToByte(BYTE* Output, DWORD* Input, UINT nLength )
{
	//entry invariants
	ASSERT( nLength % 4 == 0 );
	ASSERT( !IsBadReadPtr(Output, nLength) );
	ASSERT( !IsBadReadPtr(Input, nLength/4) );
...
std::string CMD5Checksum::Final()
{
	...
	for ( int i=0; i < nMD5Size; i++) 
	{
		...
		if (lpszMD5[i] == 0) {
			...
			stringEx::Format(Str, "0%x", lpszMD5[i]);
		}
		else {
			stringEx::Format(Str, "%x", lpszMD5[i]);
		}
