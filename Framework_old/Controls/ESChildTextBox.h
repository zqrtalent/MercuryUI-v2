...
public:
	...
	void			SetBgImage				(Image* pImage, _Rect rcOmitBorder);
	void			SetAlternateTextFont	(Font* pFont)				{pFont->CopyFont(&m_alternateTextFont);};
	void			SetAlternateTextColor	(COLORREF crText)			{m_crAlternateText = crText;};
	void			SetTextFont				(Font* pFont, bool bRedraw = false);
...
private:
	...
	bool			GetTextSize				(_string* pStr, Font* pFont, _Size& szText);
	bool			ExtTextOutMy			(_DC* pDC, int x, int y, UINT nOptions, RECT* lpRect, _string sText);
	int				GetSymbolIndexByXOffset	(_string* pStr, Font* pFont, int nXTextStart,  int nXSymbolAt);
	int				GetSymbolWidthArray		(_string* pStr, Font* pFont, CDWordArray& arrSymbolWidth);

protected:
	Image					m_imageBg;				// Background image.
	_Rect					m_rcOmitBorder;			// Omit border of bg image.
	COLORREF				m_crBorder;				// Border color.
	COLORREF				m_crFill;				// Fill color.
	
	Font					m_textFont;				// Text font.
	Font					m_textFontOriginal;		// Text font original.
	...
	Font					m_alternateTextFont;	// Alternate text font.
	...
	BOOL					m_bCloseBox;
	_Rect					m_rcCloseBox;
	Image*					m_pImageCloseBoxNormal;
	Image*					m_pImageCloseBoxHover;
	Image*					m_pImageCloseBoxPushed;
