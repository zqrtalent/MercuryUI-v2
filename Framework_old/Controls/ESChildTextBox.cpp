...
void
ESChildTextBox::SetBgImage(Image* pImage, _Rect rcOmitBorder){
	if( !m_imageBg.IsNull() )
		m_imageBg.Destroy();
...
void
ESChildTextBox::SetTextFont(Font* pFont, bool bRedraw /*= false*/){
	if( !pFont ) return;
...
ESChildTextBox::OnPaintClient(_DC *pDC, _Rect* pRectDC, _Rect* pRectClient){
	...
	if( nCt > 0 ){
		Font*	pFontOld				= pDC->SelectObject(&m_textFont);
		_Rect	rcLineDC				= rcTextAreaDC;
		int		nCurrentLine			= (m_nCursorPos >> 16);
		bool	bEmbedeedFont			= m_textFont.IsEmbedeedFontAvailable();
		int		nDistanceBetweenSymbols = bEmbedeedFont ? EmbedeedFontMan::GetDistanceBetweenSymbols() : 0;
...
//				if( ImageHelper::ExtTextOut(pDC, ptText.x, ptText.y, ETO_CLIPPED, rcTextAreaDC, sAfterSel) )
		...
		if( m_sAlternateText.length() > 0 ){
			Font* pFontOld = pDC->SelectObject(&m_alternateTextFont);
	...
	if( m_bCloseBox ){
		...
		Image* pImageBox = m_pImageCloseBoxNormal;
...
ESChildTextBox::GetTextAreaRect(){
	...
	int nTextHeight		= _min(rcTextArea.Height(), m_nTextLineHeight);
	if( m_bMultiLine )
		rcTextArea.left		+= 2;
	else{
...
ESChildTextBox::GetCursorIndexByPoint(_Point pt, int& nLineIndex){
	...
	if( nCursorIndex == -1 )
		nCursorIndex = _max((m_nLeftIndex - 1), -1);
	return nCursorIndex;
	}
...
bool
ESChildTextBox::GetTextSize(_string* pStr, Font* pFont, _Size& szText){
	bool bRet = false;
	...
	if( m_bPassword ){
		_Size	szSymbol(0, 0);
		_string sSymbol					= _T("*");
		int		nDistanceBetweenSymbols = EmbedeedFontMan::GetDistanceBetweenSymbols();
...
int
ESChildTextBox::GetSymbolIndexByXOffset(_string* pStr, Font* pFont, int nXTextStart,  int nXSymbolAt){
	int		nRet			= -1;
	bool	bEmbedeedFont	= pFont->IsEmbedeedFontAvailable();
...
int
ESChildTextBox::GetSymbolWidthArray(_string* pStr, Font* pFont, CDWordArray& arrSymbolWidth){
	int		nRet			= 0;
	int		nCt				= 0;
	bool	bEmbedeedFont	= pFont->IsEmbedeedFontAvailable();
