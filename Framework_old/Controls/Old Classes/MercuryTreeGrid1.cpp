...
BOOL 
MercuryTreeGrid::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext /*= NULL*/)
	{
...
BOOL
MercuryTreeGrid::SelectItem(MerTreeItem* pItem, BOOL bInvalidate /*= TRUE*/)
	{
	if( !pItem ) return 0;
...
BOOL
MercuryTreeGrid::ExpandItem(MerTreeItem* pItem, BOOL bCollapse /*= FALSE*/, BOOL bInvalidate /*= TRUE*/)
	{
...
BOOL
MercuryTreeGrid::CheckItem(MerTreeItem* pItem, BOOL bCheck /*= TRUE*/, BOOL bInvalidate /*= TRUE*/)
	{
	if( bInvalidate ){
		RedrawItem(pItem);
		}
...
BOOL
MercuryTreeGrid::SetItemImage(MerTreeItem* pItem, int nImage, int nImageSel, BOOL bInvalidate /*= TRUE*/)
	{
	if( !pItem ) return FALSE;
	pItem->iImage		= nImage;
...
MerTreeItem*
MercuryTreeGrid::AddItem(LPTSTR lpszText, MerTreeItem* parent /*= NULL*/, BOOL bInvalidate /*= FALSE*/)
	{
...
MerTreeItem*
MercuryTreeGrid::ItemByPoint(CPoint pt, CRect* pRcItem /*= NULL*/){
	CRect rcView;
...
MercuryTreeGrid::UpdateScrollBars(){
	...
	CRect rcVScroll(rcClient.right - m_pGUI->scrollBar.m_nVScrollWidth, rcClient.top, 
...
BOOL
MercuryTreeGrid::MoveViewPos(int nMoveX, int nMoveY, BOOL bInvalidate, 
							 BOOL bUpdateScrolls /*= TRUE*/, BOOL bRedrawAll /*= TRUE*/){
	...
	if( m_bVScroll ){
		switch( nScrollCode ){
			...
			case SB_PAGEUP:
				...
				return (m_nTopItemIndex*m_pGUI->treeGrid.m_nItemHeight) + m_ptViewOffset.y;
			case SB_PAGEDOWN:
				GetClientRectMy(rcClient);
				MoveViewPos(0, rcClient.Height(), TRUE, TRUE, FALSE);
				return (m_nTopItemIndex*m_pGUI->treeGrid.m_nItemHeight) + m_ptViewOffset.y;
			case SB_THUMBTRACK:
				return MoveViewPos(0, nPos - nPosOld, TRUE, FALSE, FALSE);
			default:
