...
ESItemContainer::ESItemContainer() : MercuryBaseView(){
	m_pView			= NULL;
	m_bView			= FALSE;
	m_pHeader		= NULL;
	m_bHeader		= FALSE;
...
IMPLEMENT_DYNCREATE(ESItemContainer, MercuryBaseView)

ESItemContainer::~ESItemContainer()
	{
	DestroyClassMembers();
	}

BEGIN_MESSAGE_MAP(ESItemContainer, MercuryBaseView)
	/*
	ON_WM_ERASEBKGND	()
	ON_WM_PAINT			()
	ON_WM_CHAR			()
...
BOOL 
ESItemContainer::Create(LPCTSTR lpszClassName,
...
void
ESItemContainer::DestroyClassMembers()
	{
	/*
	if( m_pContent ){
		delete m_pContent;
...
void
ESItemContainer::OnPrePaint(CRect& rcRepaint){
...
void 
ESItemContainer::OnPaintClient(CDC* pDC, CRect* pRectDC, CRect* pRectClient){
	if( m_nScrollsCt == 2 ){ // Scrolbars are visible
		CRect rcClient;
		GetClientRect(rcClient);
...
void
ESItemContainer::OnShowWindow(BOOL bShow, UINT nStatus){
	CWnd::OnShowWindow(bShow, nStatus);
	}

void
ESItemContainer::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags){
	CWnd::OnChar(nChar, nRepCnt, nFlags);
	}

void 
ESItemContainer::OnLButtonDown(UINT nFlags, CPoint point) {
	CWnd::OnLButtonDown(nFlags, point);
	}

void 
ESItemContainer::OnLButtonUp(UINT nFlags, CPoint point){
	CWnd::OnLButtonUp(nFlags, point);
	}

void 
ESItemContainer::OnLButtonDblClk(UINT nFlags, CPoint point){
	CWnd::OnLButtonDblClk(nFlags, point);
	}

void
ESItemContainer::OnSize(UINT nType, int cx, int cy){
...
void 
ESItemContainer::OnMouseMove(UINT nFlags, CPoint point){
	CWnd::OnMouseMove(nFlags, point);
	}

BOOL 
ESItemContainer::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message){
	return CWnd::OnSetCursor(pWnd, nHitTest, message);
	}

BOOL 
ESItemContainer::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt){
...
void
ESItemContainer::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags){
	_AFX_THREAD_STATE* pState = AfxGetThreadState();
	if( pState && m_pView )
		m_pView->SendMessage(pState->m_msgCur.message, pState->m_msgCur.wParam, pState->m_msgCur.lParam);
	else
...
UINT 
ESItemContainer::OnGetDlgCode(){
	return DLGC_WANTALLKEYS|DLGC_WANTARROWS;
	}

LRESULT	
ESItemContainer::OnVScroll(WPARAM wParam, LPARAM lParam){
...
LRESULT	
ESItemContainer::OnHScroll(WPARAM wParam, LPARAM lParam){
	if( m_pView && IsWindow(*m_pView) ){
...
BOOL
ESItemContainer::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message){
...
*/
BOOL
ESItemContainer::CanSizeHeaderItem(int nItem, int nWidth){
...
void
ESItemContainer::OnSizeHeaderItem(int nItem, int nWidth, int nWidthOld){
	ASSERT(m_pHeader && IsWindow(*m_pHeader));
	if( (nWidth - nWidthOld) == 0 )
		return;

	if( m_pHeader->m_nLeftItem < 0 )
		m_pHeader->m_nLeftItem = 0;
...
void
ESItemContainer::SizeAllChildWindows(){
...
void
ESItemContainer::RecalcChildWndRects(int cx, int cy){
...
void
ESItemContainer::AdjustScrollBars(){
	CRect rcClient;
	GetClientRect(rcClient);
	if( !m_pHeader ) return;
...
void
ESItemContainer::OnGetMinMaxInfo(MINMAXINFO* lpMMI){
