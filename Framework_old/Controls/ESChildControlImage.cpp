...
bool
ESChildControlImage::SetImage(Image* pImage, bool bRedraw){
...
ESChildControlImage::ScanMultiImages(){
	...
	if( m_pOwner->IsDesignerMode() ){
		...
		while(TRUE){
			...
			Image* pImageNew = pGUI->LoadImage(sImageFile, -1, -1, true, false);
			if( pImageNew )
				m_listMultiImages.Add(pImageNew);
			else
...
ESChildControlImage::OnPaintClient(_DC *pDC, _Rect* pRectDC, _Rect* pRectClient){
	...
	if( m_bMultiImageMode ){
		...
		while( nLoop < nCt ){
			Image* p = m_listMultiImages.GetAt(nLoop);
