...
ESChildProgressCtrl::OnPaintClient(_DC *pDC, _Rect* pRectDC, _Rect* pRectClient){
	...
	Image*		pImageBg	= &m_imageProgressBarBg;
	if( pImageBg->IsNull() ){
	...
	if( pImageBg && !pImageBg->IsNull() ){
		m_pOwner->DrawBackground(pDC, pRectDC, pRectClient, rcControl, FillMode::FillMode_OmitBorderedImage, RGB(0, 0, 0), pImageBg, m_rcOmitProgressBg);
		}

	Image*		pImageFilled	= &m_imageProgressBarFilled;
	...
	if( pImageFilled && !pImageFilled->IsNull() ){
		...
		m_pOwner->DrawBackground(pDC, pRectDC, pRectClient, rcFill, FillMode::FillMode_OmitBorderedImage, RGB(0, 0, 0), pImageFilled, m_rcOmitProgressBarFilled);
		}
