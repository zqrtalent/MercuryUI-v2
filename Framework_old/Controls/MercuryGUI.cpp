...
MercuryGUI::LoadImage(const _string sImageFile, int nWidth /*= -1*/, int nHeight /*= -1*/, bool bCache /*= true*/, bool bConvertTo32bpp /*= true*/){
	...
	// Load cached image. {{
	Image* pImage = (Image*)m_arrImagesByName.GetValue(sImageFile);
	if( pImage )
		return pImage;
	// }}
	...
	// #######################################
#else
	pImage = Image::LoadImage(sImageFile, nWidth, nHeight, bConvertTo32bpp);
	if( !pImage )
		return FALSE;
/*
	pImage = new Image();
...
MercuryGUI::Destroy(){
	...
	for(int i=0; i<m_arrImagesByName.GetCount(); i++){
		Image* pImage = (Image*)m_arrImagesByName.GetValueByIndex(i);
		if( pImage ){
			delete pImage;
			}
		}
