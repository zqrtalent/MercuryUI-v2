...
bool
ESChildPreActionControl::Create(int nId, Rect rRect, MercuryBaseView* pOwner, ESChildControl* pParent){
...
void
ESChildPreActionControl::SetLabel(std::string sLabel, bool bRedraw)	{
	m_sLabel = sLabel; 
	OnLocalizeStrings();
	if( bRedraw ) 
		Redraw();
...
void	
ESChildPreActionControl::OnLButtonClick(Point pt, UINT nFlags){
	m_bActiveState = !m_bActiveState;
	SendEvent(this, ChildControlEventCode::EventCode_ControlClick, (int)m_bActiveState);
	Redraw(true);
	}
