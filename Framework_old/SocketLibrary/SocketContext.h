...
public:
	...
	static SocketContext*	BeginAccept							(SocketConnectionListener* pConnListener, SOCKET sockListen, SSL_CTX* pSSLContext,
																int nSendBuffSize, int nReceiveBuffSize, bool bKeepAlive = false, int nKeepAliveTime = 0, int nKeepAliveInterval = 0);
public:
	virtual bool			Connect								(const TCHAR* pszIp, int nPort, bool bKeepAlive = false, int nKeepAliveTime = 0, int nKeepAliveInterval = 0, SOCKET* pCopySocket = NULL, HANDLE hConnectEvent = NULL);
	virtual bool			ConnectSync							(const TCHAR* pszIp, int nPort, bool bKeepAlive = false, int nKeepAliveTime = 0, int nKeepAliveInterval = 0, SOCKET* pCopySocket = NULL);
...
protected:
	...
	bool					ReadReceivedBytes					(DWORD dwSize);
	bool					PushSendData						(BYTE* lpData, int nDataSize, bool bSSLEncrypted);
	BYTE*					PopSendData							(int& nDataSize, bool& bIsPartial, bool& bSSLEncrypted);
