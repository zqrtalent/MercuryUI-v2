...
PokerBot::PokerBot(HWND hOwnerWnd /*= NULL*/, std::string sIpAddress /*= _T("127.0.0.1")*/, int nPort /*= 20001*/, int nReconnectTime /*= 5000*/) :  
...
PokerBot::OnPostBlind(PokerPostBlindPacket* p, void* pParam){
	...
	Action(p->m_nTableId, m_nTournamentId, -1, 
		p->m_bPostBB ? PokerActionTypes::PostBB : PokerActionTypes::PostSB, 
		p->m_bPostBB ? pInfo->m_nBigBlindAmount : pInfo->m_nSmallBlindAmount);
	}
