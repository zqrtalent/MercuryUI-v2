...
Font*
_DC::SelectObject(Font* pFont){
	if( !_context )
		return NULL;
    
    Font* pFontOld = _fontSelected;
    _fontSelected = pFont;
...
_DC::CreateMemoryBitmapDC(int nBPP, UINT width, UINT height){
    ...
    //NSBitmapImageRep * r = [[NSBitmapImageRep alloc ] initWithCGImage:_dibImage];
    BYTE* ppp = (BYTE*)CGBitmapContextGetData(context);
