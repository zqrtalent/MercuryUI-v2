...
BOOL
Image::LoadImage32bpp(const std::string sImageFile){
	if( !IsNull() ){
		ASSERT(FALSE);
		return FALSE;
		}
...
Image::LoadFromBinary(LongBinary* pBinary){
	...
	HRESULT hr = ::CreateStreamOnHGlobal(*pBinary, FALSE, &lpStream);
	if( hr != S_OK )
		return FALSE;
...
Image*
Image::LoadImage(const std::string sImageFile, int nWidth /*= -1*/, int nHeight /*= -1*/, bool bConvert32bpp /*= true*/){
	std::string sImageFileFull	= sImageFile;
	Image*		pImage			= NULL;
	...
	if( !pImage ){
		...
		LPCSTR	lpszImageFile	= sImageFileFull.c_str();
		int		nLen			= sImageFileFull.length();
		LPCTSTR lpszExt			= &lpszImageFile[nLen - 4];

		if( !pImage->LoadImage32bpp(sImageFileFull.c_str()) ){
			delete pImage;
			return NULL;
			}
		}
	...
	if( nWidth > 0 && nHeight > 0 ){
		...
		if( bmStretch.CreateDIBBitmap(pImage->GetBPP(), (COLORREF)0L, nWidth, nHeight, NULL) ){
			...
			if( pImage->ScaleBitmap_Bilinier(nWidth, nHeight, Rect(0, 0, 0, 0), stretchImageDef, (pImage->GetBPP() == 32), 255, false) ){
				pImage->Destroy();
...
BOOL
Image::Save(const std::string sImageFile){
	return FALSE;
	}
...
BOOL
Image::GetImageFromImage(Rect rcDest, Image& image){
...
Image::BitmapOnBitmap(int xDest, int yDest, ImageDef hBmpSrc, int xSrc, int ySrc, int nWidthSrc, int nHeightSrc, 
	...
	*/
	...
	Rect rcDest	(xDest, yDest, xDest + nDrawWidth, yDest + nDrawHeight);
	Rect rcSrc		(xSrc, ySrc, min(xSrc+nWidthSrc, pbmpSrc->bmWidth), min(ySrc+nHeightSrc, pbmpSrc->bmHeight));
	...
	Rect rcDrawDest(rcDest.left, rcDest.top, rcDest.left + rcSrc.Width(), rcDest.top + rcSrc.Height());
...
Image*
Image::CombineImages(Image* pImageBg, Image* pImageFg, bool bFgIsAlphaChannelImage, int nCombinedImageBPP /*= 24*/, Rect rcForeground /*= Rect(0, 0, 0, 0)*/){
	...
	Rect rcDraw(0, 0, pImageBg->GetWidth(), pImageBg->GetHeight());
	HDC srcDC = ::CreateCompatibleDC(NULL);
	...
	if( bFgIsAlphaChannelImage ){
		Size szBg(pImageBg->GetWidth(), pImageBg->GetHeight());
		Size szFg(pImageFg->GetWidth(), pImageFg->GetHeight());
...
BOOL
Image::CopyAlphaChannel(ImageDef hDest, Rect rcSrc /*= Rect(0, 0, 0, 0)*/, bool bCheckDestPixelAlpha /*= false*/, BYTE btMaxDestPixelAlphaChange /*= 255*/, int nSrcAlphaPixel /*= -1*/){
	if( IsNull() ){
		ASSERT(FALSE);
		return FALSE;
		}
	...
	if( rcSrc.IsRectEmpty() ){
		...
		Rect rcDestImg	(0, 0, bmDest.bmWidth, bmDest.bmHeight);
		Rect rcSrcImg	(0, 0, bmSrc.bmWidth, bmSrc.bmHeight);
...
BOOL
Image::RenderImage_OmitBorder(_DC* pDC, const Rect rcClipDestDC, const Rect rcDestDC, Rect rcOmitBorder, BOOL bAlpha /*= TRUE*/, BOOL bSkipMiddleCenterPart /*= FALSE*/){
	if( !pDC || IsNull() )
		return FALSE;
...
BOOL
Image::RenderImage(_DC* pDC, const Rect rcClipDestDC, const Rect rcDestDC, bool bScale /*= true*/, bool bUseAlphaChannel /*= true*/, 
	...
	if( bScale ){
		ImageDef	hDestBmp = 0;
		Image		bmBkgnd;
		Size		szClipDesDC(rcClipDestDC.Width(), rcClipDestDC.Height());
		if( bUseAlphaChannel ){
		...
		Rect rcOmitBorder	((szImage.cx-2)/2, (szImage.cy-2)/2, (szImage.cx-2)/2, (szImage.cy-2)/2);
...
BOOL
Image::ScaleBitmap(ScaleMode mode, UINT nDestCX, UINT nDestCY, Rect rcInvalidDest, ImageDef& hBmpDest, bool bUseAlphaChannel, BYTE btConstantAlpha, bool bDontAffectAlphaChannel){
	if( IsNull() )
		return FALSE;
...
BOOL
Image::ScaleBitmap_NearestN(UINT nDestCX, UINT nDestCY, Rect rcInvalidDest, ImageDef& hBmpDest, bool bUseAlphaChannel, BYTE btConstantAlpha, bool bDontAffectAlphaChannel){
	if( IsNull() )
		return FALSE;
	...
	Rect rcDest(0, 0, nDestCX, nDestCY);
...
BOOL
Image::ScaleBitmap_Bilinier(UINT nDestCX, UINT nDestCY, Rect rcInvalidDest, ImageDef& hBmpDest, bool bUseAlphaChannel, BYTE btConstantAlpha, bool bDontAffectAlphaChannel){
	if( IsNull() )
		return FALSE;
...
BOOL
Image::DrawImage32bpp_Omitborder(_DC* pDC, Rect rcDestDC, Rect rcClipDC, Rect rcOmitBorder, BOOL bAlpha, BOOL bSkipMiddleCenterPart){
	...
	Rect rcTopLeftPart			(rcDestDC.left, rcDestDC.top, rcDestDC.left + rcOmitBorder.left, rcDestDC.top + rcOmitBorder.top);
	Rect rcTopCenterPart		(rcTopLeftPart.right, rcDestDC.top, rcDestDC.right - rcOmitBorder.right, rcDestDC.top + rcOmitBorder.top);
	Rect rcTopRightPart			(rcTopCenterPart.right, rcDestDC.top, rcDestDC.right, rcDestDC.top + rcOmitBorder.top);

	Rect rcBottomLeftPart		(rcDestDC.left, rcDestDC.bottom - rcOmitBorder.bottom, rcDestDC.left + rcOmitBorder.left, rcDestDC.bottom);
	Rect rcBottomCenterPart		(rcTopLeftPart.right, rcDestDC.bottom - rcOmitBorder.bottom, rcDestDC.right - rcOmitBorder.right, rcDestDC.bottom);
	Rect rcBottomRightPart		(rcTopCenterPart.right, rcDestDC.bottom - rcOmitBorder.bottom, rcDestDC.right, rcDestDC.bottom);

	Rect rcMidLeftPart			(rcDestDC.left, rcTopLeftPart.bottom, rcDestDC.left + rcOmitBorder.left, rcBottomLeftPart.top);
	Rect rcMidCenterPart		(rcTopLeftPart.right, rcTopLeftPart.bottom, rcDestDC.right - rcOmitBorder.right, rcBottomLeftPart.top);
	Rect rcMidRightPart			(rcTopCenterPart.right, rcTopLeftPart.bottom, rcDestDC.right, rcBottomLeftPart.top);

	Rect rcTopLeftPartImg		(0, 0, rcOmitBorder.left, rcOmitBorder.top);
	Rect rcTopCenterPartImg		(rcTopLeftPartImg.right, 0, nSrcCX - rcOmitBorder.right, rcOmitBorder.top);
	Rect rcTopRightPartImg		(rcTopCenterPartImg.right, 0, nSrcCX, rcOmitBorder.top);

	Rect rcBottomLeftPartImg	(0, nSrcCY - rcOmitBorder.bottom, rcOmitBorder.left, nSrcCY);
	Rect rcBottomCenterPartImg	(rcBottomLeftPartImg.right, nSrcCY - rcOmitBorder.bottom, nSrcCX - rcOmitBorder.right, nSrcCY);
	Rect rcBottomRightPartImg	(rcBottomCenterPartImg.right, nSrcCY - rcOmitBorder.bottom, nSrcCX, nSrcCY);

	Rect rcMidLeftPartImg		(0, rcTopLeftPartImg.bottom, rcTopLeftPartImg.right, rcBottomLeftPartImg.top);
	Rect rcMidCenterPartImg		(rcBottomCenterPartImg.left, rcTopLeftPartImg.bottom, rcBottomCenterPartImg.right, rcBottomLeftPartImg.top);
	Rect rcMidRightPartImg		(rcBottomRightPartImg.left, rcTopLeftPartImg.bottom, rcBottomRightPartImg.right, rcBottomLeftPartImg.top);

	Rect* pArrRects[]		= {	&rcTopLeftPart, &rcTopCenterPart, &rcTopRightPart, 
								&rcBottomLeftPart, &rcBottomCenterPart, &rcBottomRightPart,
								&rcMidLeftPart, &rcMidCenterPart, &rcMidRightPart};

	Rect* pArrRectImgs[]	= {	&rcTopLeftPartImg, &rcTopCenterPartImg, &rcTopRightPartImg, 
	...
	while( nLoop < nCt ){
		Rect* pRect		= pArrRects[nLoop];
		Rect* pRectImg	= pArrRectImgs[nLoop];
		if( !pRect->IsRectEmpty() && !pRectImg->IsRectEmpty() ){
			Rect rcInvalid = *pRect&rcClipDC;
