...
public:
	...
	BOOL		ExtTextOut					(int x, int y, UINT options, LPCRECTDef lprect, std::string& text, const int * lpDx = NULL);
	BOOL		ExtTextOutW					(int x, int y, UINT options, LPCRECTDef lprect, const wchar_t* lpString,  UINT c, const int * lpDx = NULL);
	BOOL		DrawText					(FONTDef pFont, int x, int y, int flag, Rect rcClipRect, std::string* pszText, float fStretchCX, float fStretchCY);

	BOOL		GetTextSizeEx				(std::string* pStr, Size& szText);
	BOOL		GetTextSizeW				(wchar_t* pwszText, int nLen, Size& szText);
	BOOL		GetTextExtentPoint32		(std::string* pStr, Size& szText);
	BOOL		GetTextExtentPoint32W		(const wchar_t* lpString, int length, Size& szText);
