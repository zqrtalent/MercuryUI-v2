...
BOOL
Font::GetTextSize(std::string* pStr, Size& szText){
	if( IsNull() )
		return FALSE;
...
int
Font::GetSymbolIndexByXOffset(std::string* pStr, int nXTextStart, int nXSymbolAt){
...
int
Font::GetSymbolWidthArray(std::string* pStr, CDWordArray& arrSymbolWidth){
	if( IsNull() || !pStr || !pStr->length() )
		return 0;
