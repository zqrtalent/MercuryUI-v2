...
public:
	...
	static	Image*	LoadImage				(const std::string sImageFile, int nWidth = -1, int nHeight = -1, bool bConvert32bpp = true);
	...
	BOOL			GetImageFromImage		(Rect rcDest, Image& image);
	...
	static Image*	CombineImages			(Image* pImageBg, Image* pImageFg, bool bFgIsAlphaChannelImage, int nCombinedImageBPP = 24, Rect rcForeground = Rect(0, 0, 0, 0));
	BOOL			CopyAlphaChannel		(ImageDef hDest, Rect rcSrc = Rect(0, 0, 0, 0), bool bCheckDestPixelAlpha = false, BYTE btMaxDestPixelAlphaChange = 255, int nSrcAlphaPixel = -1);
	...
	BOOL			RenderImage_OmitBorder	(_DC* pDC, const Rect rcClipDestDC, const Rect rcDestDC, Rect rcOmitBorder, BOOL bAlpha = TRUE, BOOL bSkipMiddleCenterPart = FALSE);
	BOOL			RenderImage				(_DC* pDC, const Rect rcClipDestDC, const Rect rcDestDC, bool bScale = true, bool bUseAlphaChannel = true, 
	...
	// Image scaling.
	BOOL			ScaleBitmap				(ScaleMode mode, UINT nDestCX, UINT nDestCY, Rect rcInvalidDest, ImageDef& hBmpDest, bool bUseAlphaChannel, BYTE btConstantAlpha = 255, bool bDontAffectAlphaChannel = false);
	BOOL			ScaleBitmap_NearestN	(UINT nDestCX, UINT nDestCY, Rect rcInvalidDest, ImageDef& hBmpDest, bool bUseAlphaChannel, BYTE btConstantAlpha = 255, bool bDontAffectAlphaChannel = false);
	BOOL			ScaleBitmap_Bilinier	(UINT nDestCX, UINT nDestCY, Rect rcInvalidDest, ImageDef& hBmpDest, bool bUseAlphaChannel, BYTE btConstantAlpha = 255, bool bDontAffectAlphaChannel = false);

	BOOL			MultiplyBlendImageBpp32	(COLORREF crBlend);
	BOOL			Rotate90Bitmap32bpp		(Image& imageRotated);
	BOOL			Rotate90Bitmap32bpp		();

	BOOL			DrawImage32bpp_Omitborder(_DC* pDC, Rect rcDestDC, Rect rcClipDC, Rect rcOmitBorder, BOOL bAlpha = TRUE, BOOL bSkipMiddleCenterPart = FALSE);
