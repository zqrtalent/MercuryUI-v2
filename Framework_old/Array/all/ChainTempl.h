...
public:
	...
	Elem*	FindWithCharArrayCase	(int offset, TCHAR* pszFind);
	// Finds element by one of the elements memeber which is string type.
	Elem*	FindWithCharArray		(int offset, TCHAR* pszFind);
	...
	// Inserts element or chain of elements.
	// Returns quantity of inserted elements.
	int 	Into			(Elem* pElemPrev, Elem* pElem);
	// Cuts elements chain.
	// Returns first element of cutted chain.
	Elem*	Cut				(Elem* pElemStart, int nElemCt = 1);
...
template <class Elem> Elem*	
ChainTempl<Elem>::FindWithCharArrayCase(int offset, TCHAR* pszFind)
	{
	Elem*	pElem = m_headElem.pFirstElem;
	while( pElem ){
		if( !_tcscmp((TCHAR*)((PTR_HOLDER_INTEGER*)((PTR_HOLDER_INTEGER)(pElem) + offset)), pszFind) ){
			return pElem;
			}
		pElem = (Elem*)((PTR_HOLDER_INTEGER*)((PTR_HOLDER_INTEGER)(pElem) + m_nNextPtrOffset))[0];
		}
...
template <class Elem> Elem*	
ChainTempl<Elem>::FindWithCharArray(int offset, TCHAR* pszFind)
	{
	Elem*	pElem = m_headElem.pFirstElem;
	while( pElem ){
...
template <class Elem> int 	
ChainTempl<Elem>::Into(Elem* pElemPrev, Elem* pElem)
	{
	return 0;
	}

template <class Elem> Elem*	
ChainTempl<Elem>::Cut(Elem* pElemStart, int nElemCt /*= 1*/)
	{
	return NULL;
	}
...
ChainTempl<Elem>::SetCurrent(Elem* pElem)
	{
	}
	// Deletes all elements.
	// Returns deleted elem count.
