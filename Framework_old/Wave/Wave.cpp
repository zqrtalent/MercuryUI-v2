...
BOOL 
CWave::Open(SHORT channels, DWORD sampleRate, SHORT bitsPerSample){
	BOOL bResult = TRUE;
	// Open output device
...
BOOL 
CWave::Load(const TCHAR* lpszFilePath){
	if( !m_sFileLoaded.compare(lpszFilePath) ){
		waveOutSetVolume(m_hWaveout, m_dwVolume);
		return TRUE;
		}
	...
	// Load .WAV file
	FILE* file = NULL;
	fopen_s(&file, lpszFilePath, _T("rb"));
	if (file != NULL){
		// Read .WAV descriptor
	...
	if( bResult )
		m_sFileLoaded = lpszFilePath;
	return bResult;
	}

BOOL 
CWave::Save(const TCHAR* lpszFilePath){
	BOOL bResult = FALSE;
	// Save .WAV file
	FILE* file = NULL;
	fopen_s( &file, lpszFilePath, _T("wb"));
	if (file != NULL){
		// Save .WAV descriptor
...
void
CWave::SetVolume(BYTE btVolLeft, BYTE btVolRight){
	btVolLeft		= min(max(btVolLeft, 0), 100);
	btVolRight		= min(max(btVolRight, 0), 100);
	WORD wVolLeft	= (WORD)(0xFFFF * (float)(btVolLeft / 100.0f));
	WORD wVolRight	= (WORD)(0xFFFF * (float)(btVolRight / 100.0f));
	m_dwVolume		= (((DWORD)wVolRight)<<16) | (((DWORD)wVolLeft)&0xFFFF);
	if( !m_bStopped ){
		waveOutSetVolume(m_hWaveout, m_dwVolume);
		}
	}
...
BOOL 
CWave::WaveOut_Proc(HWAVEOUT hwi, UINT uMsg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2){
	BOOL bResult = TRUE;
	// Get current object
