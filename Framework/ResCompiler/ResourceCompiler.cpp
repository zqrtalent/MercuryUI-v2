...
ResourceCompiler*
ResourceCompiler::GetCompiler()
	{
	static ResourceCompiler compiler;
	return &compiler;
	}
...
BOOL 
ResourceCompiler::ReadDefinesFromFile(_String strHdrFile, AutoSortedArray& arrDefines)
	{/*
	_String strContent;
	BOOL	bRet = FALSE;
	try
...
BOOL 
ResourceCompiler::CompileDialogScriptFile(_String strDlgScriptFile, void*& lpBuffer, UINT& nBuffSize){
	_String strDlgScript;
	BOOL	bRet = FALSE;
	try
		{
		// open header file. =============================
		CFile file_dlgscript(strDlgScriptFile, CFile::modeRead);
		UINT nFSize = (UINT)file_dlgscript.GetLength();
		if( nFSize <= 0 )
			{
...
BOOL 
ResourceCompiler::CompileDialogScript(_String strDlgScript, void*& lpBuffer, UINT& nBuffSize, DWORD dwStyleAdd /*= 0L*/, DWORD dwStyleExAdd /*= 0L*/){
	...
	if( dwStyleAdd )
		dlgData.nStyle		= dwStyleAdd;
	if( dwStyleExAdd )
		dlgData.nStyleEx	= dwStyleExAdd;

	// Compile each item of dialog template. ======================
	typedef BOOL (*Parse_DlgItem)	(ResourceCompiler* pCompiler, LPTSTR& lpszScript, void*& lpBuffer, UINT& nBuffSize, _String& strError);
...
BOOL		
ResourceCompiler::ReadDialogScriptHeader(DialogData* pDlgData, LPTSTR& lpszScript){
	_String strWord;
	int		nFind			= -1;
...
BOOL
ResourceCompiler::GetLogFontValue(LPTSTR& lpszScript, LOGFONT& lfValue)
	{
	_String strValue;
	// Read font size. ===================================
	...
	if( GetNextWord(lpszScript, strValue) && strValue == _T(",") )
		...
		strcpy_s(lfValue.lfFaceName, strFaceName);
		}
	else
		{
...
BOOL
ResourceCompiler::GetRectValue(LPTSTR& lpszScript, _Rect& rcValue)
	{
	_String strValue;
	_Point	ptLeftTop, ptRightBottom;
...
BOOL		
ResourceCompiler::GetPointValue(LPTSTR& lpszScript, _Point& ptValue)
	{
	_String strValue;
	long	lNumValue = 0;
...
BOOL		
ResourceCompiler::GetDefineValue(LPTSTR& lpszScript, UINT& nDefineValue, AutoSortedArray& arrDefines)
	{
	// Retrieve define's as string. ======
	_String strDefineString;
...
BOOL
ResourceCompiler::GetNumericValue(LPTSTR& lpszScript, long& nNumericValue)
	{
	// retrieve next word. ================
	_String strWord;
	if( !GetNextWord(lpszScript, strWord) )
...
BOOL
ResourceCompiler::GetStringValue(LPTSTR& lpszScript, _String& sStringValue)
	{
...
BOOL 
ResourceCompiler::GetNextWord(LPTSTR& lpszScript, _String& strScriptWord)
	{
...
BOOL		
ResourceCompiler::DefineStringToValue(_String& strDefineString, UINT& nDefineValue, AutoSortedArray& arrDefines)
	{
	...
	do
		...
		_String strDefine = strDefineString.Mid(nFindOrSymbolOld + 1, nFindOrSymbol - nFindOrSymbolOld - 1);
		strDefine.Remove('\t');
		strDefine.Remove(' ');
		...
		if( strDefine[0] >= '0' && strDefine[0] <= '9' )
			...
			if( nFindValue != -1 )
				...
				if( !m_bDefineAsString )
					{
					TRACE(__FUNCTION__);
					TRACE1("%s undefined", strDefine.GetBuffer());
					}
				else
					{
					arrDefines.Add((void*)nCode, (void*)m_nDefineValue);
...
void
ResourceCompiler::AddStyleDefine(_String sDefName, UINT nValue)
	{
	sDefName.MakeUpper();
...
BOOL 
ResourceCompiler::ReadDefines(_String& strHdrFileContent, AutoSortedArray& arrDefines)
	{
	// parse resource header file. ===================
	LPTSTR	lpszContent	= strHdrFileContent.GetBuffer();
	_String	strWord, strDefineName;
	...
	while( GetNextWord(lpszContent, strWord) )
		...
		if( !bDefineCmd )
			...
			#endif
				arrDefines.Add((void*)nCode, (void*)nDefValue);
...
BOOL
ResourceCompiler::AddDefine(_String sDefineName, UINT nValue)
	{
	DWORD dwCode = DecodeString(sDefineName.GetBuffer());
	if( !dwCode || !nValue )
		return FALSE;
	m_arrDefineName_Value.Add((void*)dwCode, (void*)nValue);
	return TRUE;
	}

BOOL 
ResourceCompiler::BuildDlgTemplate(DialogData* pDlgData, void*& lpBuffer, UINT& nBuffSize)
	{
...
BOOL 
ResourceCompiler::BuildItemTemplate(DlgItemData* pItemData, void*& lpBuffer, UINT& nBuffSize)
	{
...
BOOL 
ResourceCompiler::Parse_DlgItem_PUSHBUTTON(ResourceCompiler* pCompiler, LPTSTR& lpszScript, void*& lpBuffer, UINT& nBuffSize, _String& strError)
	{
	...
	return ResourceCompiler::BuildItemTemplate(&itemData, lpBuffer, nBuffSize);
error:
	return FALSE;
	}

BOOL 
ResourceCompiler::Parse_DlgItem_EDITTEXT(ResourceCompiler* pCompiler, LPTSTR& lpszScript, void*& lpBuffer, UINT& nBuffSize, _String& strError)
	{
	LPTSTR		lpszScriptOld;
	_String		strWord;
	DlgItemData itemData;
...
BOOL 
ResourceCompiler::Parse_DlgItem_COMBOBOX(ResourceCompiler* pCompiler, LPTSTR& lpszScript, void*& lpBuffer, UINT& nBuffSize, _String& strError)
	{
	return FALSE;
	}

BOOL 
ResourceCompiler::Parse_DlgItem_LISTBOX(ResourceCompiler* pCompiler, LPTSTR& lpszScript, void*& lpBuffer, UINT& nBuffSize, _String& strError)
	{
	return FALSE;
	}

BOOL 
ResourceCompiler::Parse_DlgItem_GROUPBOX(ResourceCompiler* pCompiler, LPTSTR& lpszScript, void*& lpBuffer, UINT& nBuffSize, _String& strError)
	{
	LPTSTR		lpszScriptOld;
	_String		strWord;
	DlgItemData itemData;
...
BOOL 
ResourceCompiler::Parse_DlgItem_LTEXT(ResourceCompiler* pCompiler, LPTSTR& lpszScript, void*& lpBuffer, UINT& nBuffSize, _String& strError)
	{
	LPTSTR		lpszScriptOld;
	_String		strWord;
	DlgItemData itemData;
...
BOOL 
ResourceCompiler::Parse_DlgItem_SCROLLBAR(ResourceCompiler* pCompiler, LPTSTR& lpszScript, void*& lpBuffer, UINT& nBuffSize, _String& strError)
	{
	return FALSE;
	}

BOOL 
ResourceCompiler::Parse_DlgItem_CONTROL(ResourceCompiler* pCompiler, LPTSTR& lpszScript, void*& lpBuffer, UINT& nBuffSize, _String& strError)
	{
	LPTSTR		lpszScriptOld;
	_String		strWord;
	DlgItemData itemData;
