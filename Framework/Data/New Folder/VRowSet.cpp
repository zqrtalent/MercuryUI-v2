...
VirtualRowSet* 
VirtualRowSet::CreateFromSerializable(Serializable* pObject, CStringVector* pArrDirtyFieldNames /*= NULL*/, VirtualRowSet* pRowSet /*= NULL*/, bool bUseDbFieldName /*= true*/, int nStringLength /*= 64*/){
	ASSERT(pObject);
	...
	if( pArrDirtyFieldNames && pArrDirtyFieldNames->size() ){
		...
		if( bUseDbFieldName ){
			while( nLoop < nCt ){
				...
				if( fType == FieldType::FTNone || pInfo->m_sDbFieldName.IsEmpty() ){
					nLoop ++;
					continue;
					}

				if( fType == FieldType::FTChar )
					pResult->AddFieldEx((TCHAR*)pInfo->m_sDbFieldName.GetString(), fType, nStringLength);
				else
					pResult->AddField((TCHAR*)pInfo->m_sDbFieldName.GetString(), fType);
				nLoop ++;
				}
			}
		else{
			while( nLoop < nCt ){
				...
				if( fType == FieldType::FTChar )
					pResult->AddFieldEx((TCHAR*)pInfo->m_sAttributeName.GetString(), fType, nStringLength);
				else
					pResult->AddField((TCHAR*)pInfo->m_sAttributeName.GetString(), fType);
				nLoop ++;
				}
			}
		}
		...
		if( bUseDbFieldName ){
			while( !pInfo[nLoop].m_sAttributeName.IsEmpty() ){
				...
				if( fType == FieldType::FTNone || pInfo[nLoop].m_sDbFieldName.IsEmpty() ){
					nLoop ++;
					continue;
					}

				if( fType == FieldType::FTChar )
					pResult->AddFieldEx((TCHAR*)pInfo[nLoop].m_sDbFieldName.GetString(), fType, nStringLength);
				else
					pResult->AddField((TCHAR*)pInfo[nLoop].m_sDbFieldName.GetString(), fType);
				nLoop ++;
				}
			}
		else{
			while( !pInfo[nLoop].m_sAttributeName.IsEmpty() ){
				...
				if( fType == FieldType::FTChar )
					pResult->AddFieldEx((TCHAR*)pInfo[nLoop].m_sAttributeName.GetString(), fType, nStringLength);
				else
					pResult->AddField((TCHAR*)pInfo[nLoop].m_sAttributeName.GetString(), fType);
				nLoop ++;
				}
			}
		}
...
VirtualRowSet::GetInstanceCopy(){
	...
	for(int i=0; i<m_pFields->GetCount(); i++){
		...
		_tcscpy_s(pFieldCopy->szName, pField->szName);
	...
	pRowsetCopy->m_pWndOwner	= m_pWndOwner;
...
bool
VirtualRowSet::SortByField(TCHAR* pszName, int& nCurrentRow, bool bAsc /*= true*/){
	int					nLoop		= 0;
	int					nCt			= GetRowCount();
	...
	// Sort of rows.
	VRowHeader* pRowFirst	= NULL,  *pRow = NULL, *pPrev = NULL;
	for( int i=0; i<arrRowItems.GetCount(); i++ ){
...
bool
VirtualRowSet::SetFieldNull(TCHAR* pszName){
...
bool
VirtualRowSet::IsFieldNull(TCHAR* pszName){
	FIELD_LENGTH_TYPE* pnLen = GetFieldLengthPtr(pszName);
	if( !pnLen )
		return false;
	return (*pnLen == -1);
...
int
VirtualRowSet::FindRow_ByIntField(int nFieldIdx, int nValue){
	if( nFieldIdx >= GetFieldCount() || !m_pFields )
		return -1;
	VRowSetFieldInfo* pFieldInfo = GetFieldInfo(nFieldIdx);
	if( !pFieldInfo || pFieldInfo->type != FieldType::FTInt32 )
		return -1;

	VRowHeader* pCurrRow = m_pRow;
	int			nCurrRow = m_nCurrentRow;

	int nLoop = 0, nCt = m_nRowCt, nResult = -1;
	while( SetCurrentRow(nLoop) ){
		if( GetInt32(nFieldIdx) == nValue ){
			nResult = nLoop;
			break;
			}
		nLoop ++;
...
bool
VirtualRowSet::SetDate(TCHAR* pszName, DateTime dtVal){
	int nFieldIdx = GetOrdinalIndex(pszName);
	if( nFieldIdx == -1 ) return false;
...
bool
VirtualRowSet::SetDateTime(TCHAR* pszName, DateTime dtVal){
	int nFieldIdx = GetOrdinalIndex(pszName);
	if( nFieldIdx == -1 ) return false;
...
bool
VirtualRowSet::SetString(TCHAR* pszName, _String sVal){
	int nFieldIdx = GetOrdinalIndex(pszName);
	if( nFieldIdx == -1 ) return false;
...
bool	
VirtualRowSet::GetDate(TCHAR* pszName, DateTime& dtDate){
	int nFieldIdx = GetOrdinalIndex(pszName);
	if( nFieldIdx == -1 ) return false;
...
bool	
VirtualRowSet::GetDateTime(TCHAR* pszName, DateTime& dtDate){
	int nFieldIdx = GetOrdinalIndex(pszName);
	if( nFieldIdx == -1 ) return false;
...
bool	
VirtualRowSet::GetString(TCHAR* pszName, _String& strValue){
	int nFieldIdx = GetOrdinalIndex(pszName);
	if( nFieldIdx == -1 ) return false;
