...
DataFieldCopier::CopyIntoVRow_Binary(VirtualRowSet* pVRowSet, Serializable* pDataObject, CopyFieldInfo* pCopyInfo){
	CLongBinary*	pLongBinary =  (CLongBinary*)(&((BYTE*)pDataObject)[pCopyInfo->pVarInfo->m_nOffset]);
	SIZE_T			nSize		= ::GlobalSize(pLongBinary->m_hData);
	void*			lpData		= ::GlobalLock(pLongBinary->m_hData);
