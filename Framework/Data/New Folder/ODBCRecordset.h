...
public:
	...
	CDBField& operator =( const _String& cVal );
	...
	//	Get the _String value. Do appropriate conversion.
...
private:
	...
	_String		m_cName;	//	Contain the field (column) name
...
/////////////////////////////////////////////////////////////////////////////
inline
const	_String&	CDBField::GetName()	const {
	return	m_cName;
}
...
public:
	...
	_String	        GetDateFormatted( LPCTSTR szName );

	virtual bool			GetBool( LPCTSTR szName );
	virtual unsigned char	GetChar( LPCTSTR szName );
	virtual short			GetShort( LPCTSTR szName );
	virtual int				GetInt( LPCTSTR szName );
	virtual long			GetLong( LPCTSTR szName );
	virtual float			GetFloat( LPCTSTR szName );
	virtual double			GetDouble( LPCTSTR szName );
	virtual COleDateTime	GetDate( LPCTSTR szName );
	virtual _String			GetString( LPCTSTR szName );
	virtual CLongBinary*	GetBinary( LPCTSTR szName );
	...
	_String			GetString( int nCol );
	CLongBinary*	GetBinary( int nCol );

	const _String	GetLastSqlSource();
	void			SetLastSqlSource(_String sSQL);
...
private:
	...
	_String				m_strLastSQL;			// last queried sql source.
	...
	static int			GetVariantType	(short nSQLType);
};
/////////////////////////////////////////////////////////////////////////////
...
//
inline
_String	CODBCRecordset::GetDateFormatted( LPCTSTR szName )
	{
	_String sFmtDate=_T("");
	
	COleDateTime dtSearch = Field( szName ).AsDate(); 
	
	if( dtSearch.GetStatus() == COleDateTime::valid )
		{
		sFmtDate.Format("%02d/%02d/%04d",dtSearch.GetDay(), dtSearch.GetMonth(),
		dtSearch.GetYear());
		}
/*
	sTmpDate.TrimRight();
	...
	sFmtDate.Format("%s/%s/%s", sTmpDate.Mid(6,2), sTmpDate.Mid(4,2), sTmpDate.Left(4));
*/
	return	sFmtDate;
	}
...
inline
_String	CODBCRecordset::GetString( LPCTSTR szName ) {
...
inline
_String	CODBCRecordset::GetString( int nCol ) {
...
inline const _String			
CODBCRecordset::GetLastSqlSource()
	{
	return m_strLastSQL;
	}

inline void			
CODBCRecordset::SetLastSqlSource(_String sSQL)
	{
	m_strLastSQL = sSQL;
	}
