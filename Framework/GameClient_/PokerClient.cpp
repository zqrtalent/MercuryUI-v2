...
PokerClient::PokerClient(HWND hOwnerWnd /*= NULL*/, _String sIpAddress /*= _T("127.0.0.1")*/, int nPort /*= 20001*/, int nReconnectTime /*= 5000*/) :  
...
void
PokerClient::SeatAt(int nTableId, int nSeatIndex, INT_CHIPS nAmount){
...
void
PokerClient::AddChips(int nTableId, INT_CHIPS nChipAmount){
	PokerAddChipsPacket* packet = new PokerAddChipsPacket();
	packet->ZeroPacketVariables();
	packet->m_nTableId			= nTableId;
...
void
PokerClient::Action(int nTableId, int nTournamentId, int nSeat, PokerActionTypes action, INT_CHIPS nAmount){
...
void
PokerClient::SetOption(int nTableId, int nTournamentId, PokerTableOption option, bool bValue, INT_CHIPS nValue /*= 0*/, int nSeatIndex /*= 0*/){
...
bool
PokerClient::UploadAvatarImageFile(_String sImageFile, CObject* pReceiver, UploadProgressProc procProgress, UploadProgressProc procComplete, UploadProgressProc procError){
	Image image;
	if( IsAvatarUploadStarted() )
		return false;
	_String sImage = sImageFile.GetBuffer();
	if( !image.LoadImage32bpp(sImage) )
		return false;
...
bool
PokerClient::UploadAvatarImage(Image* pImgAvatar, CObject* pReceiver, UploadProgressProc procProgress, UploadProgressProc procComplete, UploadProgressProc procError){
...
bool
PokerClient::Rebuy(int nTableId, int nTournamentId, bool bByVPPAmount /*= false*/, bool b2xRebuy /*= false*/){
	...
	p->m_nTableId			= nTableId;
	p->m_nTournamentId		= nTournamentId;
	p->m_bByVPPAmount		= bByVPPAmount;
	p->m_btRebuyMultiplier	= (b2xRebuy ? 2 : 1);
	bool bRet = SendPacket(p);
	delete p;
	return bRet;
	}
...
bool
PokerClient::Addon(int nTableId, int nTournamentId, bool bByVPPAmount /*= false*/){
	if( nTableId <=0 || nTournamentId <= 0 || !IsLoggedIn() )
		return false;
...
bool
PokerClient::SendChatMessage(int nTableId, int nTournamentId, _String sMessage){
	if( !IsConnected() || !IsLoggedIn() || sMessage.IsEmpty() )
		return false;
...
bool
PokerClient::TransferMoney(int nAccountFrom, int nAccountTo, INT_CHIPS nAmount, bool bRefreshBalances){
...
bool
PokerClient::FindPlayer(_String sUserName){
...
bool
PokerClient::FindTournament(_String sTournamentID){
...
PokerClient::UpdateTournamentsCache(PokerGetLobbyInfoPacket* p){
	...
	const int nArrGameSubTypes[][5] = {{GameSubType::Tournament_Freeroll, GameSubType::Tournament_Guaranteed, GameSubType::Tournament_Regular, GameSubType::Tournament_SitNGo, GameSubType::Tournament_Satellite}};
				...
				if( nGameSubType&nArrGameSubTypes[i][j] ){
					...
					for(int nLoop=0; nLoop<p->m_listTournamentDynamicInfo.GetCount(); nLoop++){
						...
						if( pInfo ){
							...
							pInfo->m_bRegistrationIsOpen		= pDynInfo->m_bRegistrationIsOpen;
			...
			for(int j=0; j<5; j++){
				if( nGameSubType&nArrGameSubTypes[i][j] ){
					...
					for(int nLoop=0; nLoop<p->m_listTournamentInfo.GetCount(); nLoop++){
						...
						if( !pInfo ) continue;
						int nSubTypeTournament = (pInfo->m_shSubType&GameSubType::Tournament_All);
...
PokerClient::DownloadMultipleAvatars(AutoSortedArray* pArrUserIdByAvatarId){
	...
	int nAvatarId = 0, nUserId = 0;
	if( nCtAvatarsToDownload > 0 ){
		nAvatarId	= (int)m_arrUserIdAvatarDownload.GetData(0);
		nUserId		= (int)m_arrUserIdAvatarDownload.Get(0);
		}
	m_lockDownload.Unlock(); // Unlock
...
PokerClient::AvatarExists(int nAvatarId){
	_String sAvatarFile = GetAvatarImageFileById(nAvatarId);
	HANDLE hFile = CreateFile(sAvatarFile.GetBuffer(), GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, 0);
...
PokerClient::OnDownloadAvatar(PokerDownloadAvatarPacket* p, void* pParam){
	...
	if( p->m_imageData.GetBinarySize() > 0){
		...
		m_avatarDownloading.AddBytes(lpImageData, p->m_imageData.GetBinarySize());
	...
	if( !p->m_bCompleted ){
		...
		_String sImageFile = GetAvatarImageFileById(p->m_nAvatarId);
		HANDLE hAvatarFileNew = CreateFile(sImageFile.GetBuffer(), GENERIC_WRITE, 0, NULL, CREATE_NEW, 0, 0);
		if( hAvatarFileNew != INVALID_HANDLE_VALUE ){
			DWORD dwWritten = 0L;
			WriteFile(hAvatarFileNew, m_avatarDownloading.GetBufferPtr(), (DWORD)m_avatarDownloading.GetBufferSize(), &dwWritten, NULL);
			CloseHandle(hAvatarFileNew);
			}
...
PokerClient::OnRefreshBalance(PokerRefreshBalancePacket* p, void* pParam){
	m_nPlayBalance				= p->m_nPlayMoneyAmount;
	m_nPlayMoneyInPlayBalance	= p->m_nPlayMoneyInPlay;
	m_nBalance					= p->m_nBalance;
	m_nInPlay					= p->m_nCashInPlay;
	m_dVPPPoints				= p->m_dVPPoints;
	m_nTicketsCt				= (int)p->m_cTicketsCt;
	}
...
PokerClient::OnRegisterInTournament(PokerRegisterInTournamentPacket* p, void* pParam){
	if( p->m_nErrorCode == ES_ERROR_SUCCESS ){
		...
		if( p->m_cRegisterBy == PokerRegisterInTournamentPacket::RegisterBy_BalanceAmount )
			m_nBalance = p->m_nBalance;
		else
			if( p->m_cRegisterBy == PokerRegisterInTournamentPacket::RegisterBy_Points )
				m_dVPPPoints = p->m_dPoints;
...
PokerClient::OnRebuy(PokerRebuyPacket* p, void* pParam){
	if( p->m_nErrorCode == ES_ERROR_SUCCESS ){
		if( p->m_bByVPPAmount )
			m_dVPPPoints = (float)p->m_fVPPBalanceNew;
		else
...
PokerClient::SetAvatarId(int nAvatarId){
	...
	if( nAvatarOld != m_nAvatarId){
		...
		DeleteFile(sAvatarFile.GetBuffer());
		// }}
		}
...
PokerClient::GetAvatarImageFileById(int nAvatarId){
	...
	_String sAvatarFile;
	sAvatarFile.Format(_T("%s\\%s.jpg"),m_sAvatarsCacheFolder.GetBuffer(), sMD5.GetBuffer());
