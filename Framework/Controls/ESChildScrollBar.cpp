...
ESChildScrollBar::CalculateRect(){
	if( m_bVscroll ){
		...
		m_rcThumb.bottom	= min(m_rcThumb.top + nThumbHeight, m_rcArrowBtnBottom.bottom - m_nArrowBtnCY);
...
ESChildScrollBar::ThumbTopOffsetByScrollPos(int nThumbWidthHeight){
	...
	if( m_bVscroll ){
		...
		int	nThumbPosMax	= (m_rRect.Height() - 2*m_nArrowBtnCY) - max(nThumbWidthHeight, m_nThumbHeightMin);
		int nPosMax			= (int)((m_nMax - m_nPage) + m_nMin);
		if( nThumbPosMax <= 0 ) 
			return nThumbPos;

		double dDiff		= ((double)nThumbPosMax) / nPosMax;
		nThumbPos			= min((int)(dDiff*nPos), nThumbPosMax) + nThumbPos;
		return nThumbPos;
		}
	else{
		int nPos			= m_nPos;
		int nThumbPos		= m_rRect.left + m_nArrowBtnCX;
		int	nThumbPosMax	= (m_rRect.Width() - 2*m_nArrowBtnCX) - max(nThumbWidthHeight, m_nThumbHeightMin);
		int nPosMax			= (int)((m_nMax - m_nPage) + m_nMin);
		if( nThumbPosMax <= 0 ) 
			return nThumbPos;
