...
// Properties
public:
    int   GetCaption   (_String& sCaption) const;
    int   GetShortCut  (_String& sShortCut) const;
...
///////////////////////////////////////////////////////////////////////////////
CMenuItem::CMenuItem (){
    memset (&m_miInfo, 0, sizeof(MENUITEMINFOW));
	}

///////////////////////////////////////////////////////////////////////////////
CMenuItem::CMenuItem (HMENU hMenu, UINT uItem, bool fByPosition){
    memset (&m_miInfo, 0, sizeof(MENUITEMINFOW));
    m_miInfo.cbSize = sizeof(MENUITEMINFOW);
    m_miInfo.fMask = MIIM_STATE|MIIM_SUBMENU|MIIM_TYPE|MIIM_DATA|MIIM_ID;
    VERIFY (::GetMenuItemInfoW (hMenu, uItem, fByPosition, &m_miInfo));
...
///////////////////////////////////////////////////////////////////////////////
int 
CMenuItem::GetCaption (_String& sCaption) const{
...
///////////////////////////////////////////////////////////////////////////////
int 
CMenuItem::GetShortCut (_String& sShortCut) const{
    ASSERT(m_miInfo.fMask & MIIM_TYPE);
    _String sCaption = m_sCaption;
    int nShortCutPos = sCaption.Find ('\t');
    if ( nShortCutPos == -1 ){
        sShortCut = "";
        return 0;
...
///////////////////////////////////////////////////////////////////////////////
int 
CMenuItem::GetCaptionWidth (CDC* pDC) const{
    if( GetSeparator())
        return 0;
    _String sCaption;
    int nLength = 0;
    if( GetCaption (sCaption) > 0 ){
		...
		int nLen = StringHelper::UTF8ToUnicode(&sCaption, wszUnicode, 255);
		_Size szText;

		_DC dc;
		dc.Attach(pDC->m_hDC);
		dc.GetTextSizeW(wszUnicode, nLen, szText);
		dc.Detach();
        nLength = szText.cx;
		}
    return nLength;
...
///////////////////////////////////////////////////////////////////////////////
int 
CMenuItem::GetShortCutWidth (CDC* pDC) const{
    if( GetSeparator() )
        return 0;
...
///////////////////////////////////////////////////////////////////////////////
int 
CMenuItem::GetHeight (CDC* pDC) const{
    TEXTMETRIC tm;
...
///////////////////////////////////////////////////////////////////////////////
bool 
CMenuItem::Draw(CDC* pDC, LPCRECT pRect, bool bSelected, bool bMenuBar, bool bHotLight, bool bInactive, bool bNoAccel) const{
	...
	if ( bSelected || (bHotLight && !bInactive) ){
    ...
    if ( GetSeparator() ){
        ...
        if( GetCaption (sCaption) > 0 ){
			...
			_Size szText(0, 0);
			dc.GetTextSizeEx(&sCaption, szText);
			_Point ptText;
			ptText.y = rc.top + (rc.Height() - szText.cy)/2;

            if ( bMenuBar ){
                rc.right -= TEXTPADDING_MNUBR;
				ptText.x = rc.left + (rc.Width() - szText.cx)/2;
                //pDC->DrawText (sCaption, rc, DT_SINGLELINE|DT_VCENTER|DT_CENTER|dwHidePrefix);
				dc.ExtTextOut(ptText.x, ptText.y, 0L, &rc, sCaption);
				}
            else{
                ...
                if ( GetShortCut (sShortCut) > 0 ){
					...
					ptText.x	= rc.right - szText.cx;
                    //pDC->DrawText (sShortCut, rc, DT_SINGLELINE|DT_VCENTER|DT_RIGHT);
					dc.ExtTextOut(ptText.x, ptText.y, 0L, &rc, sShortCut);
					}

                if ( GetChecked() ){
                    COLORREF crHighLight = ::GetSysColor (COLOR_HIGHLIGHT);
                    CPenDC pen (*pDC, crHighLight);
                    CBrushDC brush (*pDC, crBackImg = GetDisabled() ? HLS_TRANSFORM (::GetSysColor (COLOR_3DFACE), +73, 0) : (bSelected ? HLS_TRANSFORM (crHighLight, +50, -50) : HLS_TRANSFORM (crHighLight, +70, -57)));
                    pDC->Rectangle (_Rect (pRect->left+1, pRect->top+1, pRect->left+IMGWIDTH+4, pRect->bottom-1));
					}
                ...
                if ( m_ImgDesc.m_hImgList != NULL && m_ImgDesc.m_nIndex != -1 ){
                    ...
                    if ( GetDisabled() || (bSelected && !GetChecked()) ){
                        ...
                        pDC->DrawState (_Point (pRect->left + ( bOver ? 4 : 3 ), rc.top + ( bOver ? 4 : 3 )),
                                        _Size (IMGWIDTH, IMGHEIGHT), hIcon, DSS_MONO,
                    ...
                    if ( GetRadio() ){
						_Point		ptCenter(rc.left + rc.Width()/2, rc.top + rc.Height()/2);
                        ...
                        pDC->Ellipse	(_Rect (ptCenter.x-4, ptCenter.y-3, ptCenter.x+3, ptCenter.y+4));
						...
						pDC->DrawState (_Point (rc.left,rc.top+3), _Size(rc.Width(), rc.Height()), hBmp, DSS_NORMAL, (HBRUSH)NULL);
                        DeleteObject (hBmp);
						}
					}
				}
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
CMap <int, int, _String, _String&> CMenuXP::ms_sCaptions;
CMap <HMENU, HMENU, _String, _String&> CMenuXP::ms_sSubMenuCaptions;
CMap <int, int, CImgDesc, CImgDesc&> CMenuXP::ms_Images;
CMap <HMENU, HMENU, CImgDesc, CImgDesc&> CMenuXP::ms_SubMenuImages;

///////////////////////////////////////////////////////////////////////////////
void 
CMenuXP::SetXPLookNFeel(CWnd* pWnd, bool bXPLook){
    if ( bXPLook )
...
///////////////////////////////////////////////////////////////////////////////
bool 
CMenuXP::GetXPLookNFeel (const CWnd* pWnd){
    return ::GetProp (pWnd->GetSafeHwnd(), _WndPropName_MenuXP) != NULL;
	}

///////////////////////////////////////////////////////////////////////////////
void 
CMenuXP::UpdateMenuBar (CWnd* pWnd){
...
void
CMenuXP::UpdateMenuItemStrings(HMENU hMenu, bool bMenuBar){
	wchar_t wszText[100];
	...
	int nCt				= ::GetMenuItemCount(hMenu);
	_String sItemText;

	if( bMenuBar ){
		CClientDC		cDC				(AfxGetMainWnd());
		TEXTMETRIC		tm;
		FontDC			font			(cDC, _T("Arial"), 9);
		...
		for(int i=0; i<nCt; i++){
			...
			wszText[0]				= 0;
			wszText[1]				= 0;
			if( !::GetMenuItemInfoW(hMenu, i, TRUE, &miinfo) )
				continue;

			StringHelper::UnicodeToUTF8(wszText, miinfo.cch, sItemText);

			_Size szText;
			_DC dc;
			dc.Attach(cDC.m_hDC);
			dc.GetTextSizeW(wszText, miinfo.cch, szText);
			dc.Detach();
			miinfo.dwItemData	= MAKELONG(MAKEWORD(0, 0), szText.cx);
			UINT nMaskOld		= miinfo.fMask;
			miinfo.fMask		= MIIM_DATA;
			::SetMenuItemInfoW (hMenu, i, true, &miinfo);
			miinfo.fMask		= nMaskOld;
			}
		}
	...
	for(int i=0; i<nCt; i++){
		...
		if( miinfo.hSubMenu ){
			UpdateMenuItemStrings(miinfo.hSubMenu, false);
			ms_sSubMenuCaptions.SetAt (miinfo.hSubMenu, sItemText);
			}
		else
			ms_sCaptions.SetAt (miinfo.wID, sItemText);
		}
	}

///////////////////////////////////////////////////////////////////////////////
void 
CMenuXP::SetXPLookNFeel (CWnd* pWnd, HMENU hMenu, bool bXPLook, bool bMenuBar){
    if ( !bXPLook ){
    ...
    for ( int i = 0; i < nItemCount; i++ ){
        ...
        if( (mii.fType & MFT_OWNERDRAW) == 0 && (!bMenuBar || (mii.fType & MFT_BITMAP) == 0) ){
            ...
            if ( bMenuBar ){
				...
				dc.GetTextSizeW(sCaption, mii.cch, szText);
				mii.dwItemData	= MAKELONG(MAKEWORD(0, 0), szText.cx);
				//_Rect rcText (0, 0, 1000, 0);
				//cDC.DrawText (sCaption, (int)_tcslen (sCaption), rcText, DT_SINGLELINE|DT_LEFT|DT_CAL_Rect);
            ...
            if( (mii.fType & MFT_SEPARATOR) == 0 ){
                _String sBuff;
				StringHelper::UnicodeToUTF8(sCaption, mii.cch, sBuff);
...
///////////////////////////////////////////////////////////////////////////////
void 
CMenuXP::SetMRUMenuBarItem (RECT& rc){
    CMenuItem::ms_rcMRUMenuBarItem = rc;
	}

///////////////////////////////////////////////////////////////////////////////
void 
CMenuXP::SetMenuItemImage (UINT nID, HIMAGELIST hImgList, int nIndex){
...
///////////////////////////////////////////////////////////////////////////////
void 
CMenuXP::OnMeasureItem (MEASUREITEMSTRUCT* pMeasureItemStruct){
...
///////////////////////////////////////////////////////////////////////////////
bool 
CMenuXP::OnDrawItem (DRAWITEMSTRUCT* pDrawItemStruct, HWND hWnd){
    if ( pDrawItemStruct->CtlType != ODT_MENU )
        return false;

	bool		bSelected	= ((pDrawItemStruct->itemState&ODS_SELECTED)!=0);
	bool		bMenuBar	= (LOBYTE(LOWORD(pDrawItemStruct->itemData)) == 0);
    ...
    if( item.Draw(&cDC, &pDrawItemStruct->rcItem, 
		bSelected, 
		bMenuBar, 
...
///////////////////////////////////////////////////////////////////////////////
LRESULT 
CMenuXP::OnMenuChar (HMENU hMenu, UINT nChar, UINT nFlags){
...
///////////////////////////////////////////////////////////////////////////////
CWndMenuXP* 
CWndMenuXP::FromHandle(HWND hWnd, bool bPermanent){
...
///////////////////////////////////////////////////////////////////////////////
void 
CWndMenuXP::OnWindowPosChanging(WINDOWPOS* pWP){
...
///////////////////////////////////////////////////////////////////////////////
void 
CWndMenuXP::OnEraseBkgnd (){
    if( !IsWindowVisible (m_hWnd) ){
        CClientRect rc (m_hWnd);
        if ( m_bmpBkGnd.m_hObject != NULL )
            m_bmpBkGnd.DeleteObject();
        m_bmpBkGnd.Attach (GetScreenBitmap (_Rect (m_ptMenu.x, m_ptMenu.y, rc.right+m_ptMenu.x+10, rc.bottom+m_ptMenu.y+10)));
		}
	}
