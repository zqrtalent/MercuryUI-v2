...
PacketHandler::DestroyArrays(){
	...
	int nCt		= m_arrClassObjsByPacketCode.GetCount();
	int nLoop	= 0;
...
bool
PacketHandler::SetPacketWaitingFor(CEvent* pEvent, int nPacketCode, BasePacket** pPacket){
...
PacketHandler::HandlePacketByCode(int nCode, void* pParam, BYTE* lpData, int nSize, int* pnSizeBytesDeserialized /*= NULL*/){
	...
	if( nIndex != -1 ){
		...
		if( m_pEventCallback && m_nPacketWaitingFor == packet->m_nCode ){	
			CEvent* pEvent		= m_pEventCallback;
			m_pEventCallback	= NULL;
			...
			if( m_pPacketWaitingFor ){
				if( AfxIsValidAddress(m_pPacketWaitingFor, sizeof(void*)) )
