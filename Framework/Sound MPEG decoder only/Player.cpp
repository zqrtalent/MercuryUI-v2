...
SoundPlayer::Play() {
    ...
    if( Args->EndPos == 0 ) 
		SetLimits(Args->StartPos, Args->EndPos);
    fr.header_change = 0;
    
    m_hThreadQueueEvent = CreateEvent(NULL, false, false, NULL);
    if( !m_hThreadQueueEvent ){
	...
	PlayThreadHan = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)&SoundPlayer::PlayThread, this, 0, &PlayerThreadID);
    if( !PlayThreadHan ){
		LastError = peInternalError;
		return false;           
		}
    WaitForSingleObject	(m_hThreadQueueEvent, INFINITE);
    CloseHandle			(m_hThreadQueueEvent);
	m_hThreadQueueEvent	= NULL;
...
DWORD __stdcall 
SoundPlayer::PlayThread(SoundPlayer* pParam){
	DWORD nRet = pParam->PlayThreadProc();
    return nRet;
	}

DWORD
SoundPlayer::PlayThreadProc(){
	/*
    MMRESULT mmRes;
    //int clip;
    long numframes = -1;
    ...
    SoundPlayer *Player = (SoundPlayer *)ptr;    
    Args->IsVBR = false;
    try 
    {
        while(numframes) 
        {
            ...
            try
            {
                ...
                if(fr.WhatLayer==3) 
					do_layer3(); 
				else if(fr.WhatLayer==2) 
					do_layer2(); 
				else if(fr.WhatLayer==1) 
					do_layer1();
...
void 
SoundPlayer::SynthesisFilterInit(){
    typedef int (*func)(real *,int,unsigned char * , int *);
    typedef int (*func_mono)(real *,unsigned char * , int *);
    static func funcs[2][4] = {
        {synth_1to1, synth_2to1, synth_4to1, synth_ntom} ,
        {synth_1to1_8bit,synth_2to1_8bit, synth_4to1_8bit, synth_ntom_8bit}
		};
...
void 
SoundPlayer::HardAudioReset(SoundPlayer *ptr){
    FinishBuffer();
    try {
        Done_MPAudio();
		}
...
bool 
SoundPlayer::SetInName (const char *FileName){
    if( PlayerMode() != pmClosed) {
        LastError = peIncorrectMode;
        return false;
		}  
...
bool 
SoundPlayer::SetOutName(const char *FileName){
...
bool 
SoundPlayer::SetInMode (esInputMode InMode){
    if( PlayerMode() > pmClosed) {
        LastError = peIncorrectMode;
        return false;
		}
...
bool 
SoundPlayer::SetOutMode(esOutputMode OutMode){
    if( PlayerMode() >= pmPlaying) {
        LastError = peIncorrectMode;
        return false;
		}
...
bool 
SoundPlayer::SetPos(unsigned long NewPos){
    if( (PlayerMode() == pmPlaying) || (PlayerMode() == pmPaused) ){
...
bool 
SoundPlayer::SetAudioBuffers(unsigned long buffers, unsigned long size){
    if( PlayerMode() >= pmReady){
        LastError = peIncorrectMode;
        return false;        
		}
...
bool 
SoundPlayer::SetOutputDevice(unsigned long DevNum) {
    if( PlayerMode() != pmClosed) {
        LastError = peIncorrectMode;
        return false;        
		} 
...
bool 
SoundPlayer::SetLimits(unsigned long StartPos, unsigned long EndPos ){
...
bool 
SoundPlayer::SetPriority(long Priority) {
    if( PlayerMode() >= pmPlaying )
...
bool
SoundPlayer::SetInCB(InFileOpenCB openCB, InFileCloseCB closeCB, InFileGetSizeCB getsizeCB, InFileSeekCB seekCB, InFileReadCB readCB, unsigned long UserData1) {
    if( PlayerMode() != pmClosed ){
        LastError = peIncorrectMode;
        return false;
		}
...
bool 
SoundPlayer::SetOutCB(WaveOutCB OutCB, WaveOutActionCB OutActionCB, unsigned long UserData) {
    if( PlayerMode() >= pmPlaying) {
        LastError = peIncorrectMode;
        return false;
    } else {
...
bool 
SoundPlayer::CanSetVolume(bool *Separate) {
    return Audio_CanSetVolume(Separate);    
	}

bool 
SoundPlayer::SetVolume(unsigned long Volume) {
    return Audio_SetVolume(Volume);
	}

bool 
SoundPlayer::GetVolume(unsigned long *Volume){
    return Audio_GetVolume(Volume);    
	}

bool 
SoundPlayer::SetDevNum(unsigned long DevNum) {
    if( PlayerMode() >= pmPlaying ) {
        LastError = peIncorrectMode;
        return false;
		} 
...
bool 
SoundPlayer::SetWindow(HWND Wnd){
    if( PlayerMode() >= pmPlaying ){
        LastError = peIncorrectMode;
        return false;
		}
...
bool 
SoundPlayer::UseOutCB(bool UseCB){
    if( PlayerMode() >= pmPlaying ){
        LastError = peIncorrectMode;
        return false;
		}
