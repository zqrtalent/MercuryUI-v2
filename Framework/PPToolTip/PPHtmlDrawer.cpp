...
/////////////////////////////////////////////////////////////////
void 
CPPHtmlDrawer::DrawHtml(LPSIZE lpSize, LPCRECT lpRect){
...
//		m_arrTableSizes.clear();
	...
	m_lfDefault.lfItalic			= m_defStyle.bItaliFont;
...
SIZE 
CPPHtmlDrawer::DrawHtmlTable (CPPString & sTable, LPCRECT lpRect){
	...
	while (i < sHtml.GetLength())
	{
		...
		if (!sText.IsEmpty())
		{
			...
			while (!sText.IsEmpty())
			{
				...
				dc.GetTextExtentPoint32(&sText, sz);
				...
				if (MODE_DRAW == m_nNumPass)
				{
					if (sz.cx)
					{
						if ((0 == (nRealWidth - sz.cx)) && (_T(' ') == sTemp.GetAt(sTemp.GetLength() - 1)))
						{
							...
							_Size szTemp;
							dc.GetTextExtentPoint32(&sTemp, szTemp);
						...
						//ENG: Gets a size a output text
						dc.GetTextExtentPoint32(&sTemp,  sz);
						...
						dc.ExtTextOut(ptOutput.x, y, 0L, NULL, sTemp);
						nSpacesInLine -= GetCountOfChars(sTemp);
					...
					if (StoreRestoreStyle(bCloseTag))
					{
						...
						if (!bCloseTag)
						{
							...
							case TAG_BOLD:
								...
								break;
							case TAG_ITALIC:
								m_defStyle.bItaliFont = m_defStyle.bItaliFont ? FALSE : TRUE;
								break;
							case TAG_UNDERLINE:
...
void 
CPPHtmlDrawer::PrepareOutput(HDC hDC, LPCTSTR lpszHtml, LPSIZE lpSize){
...
//  Returns :
//		A pointer to a ImageList object
//
/////////////////////////////////////////////////////////////////////////////
//ImageList * CPPHtmlDrawer::GetImageList(_Size & sz)
//{
//	sz = m_szImageList;
//	return &m_ImageList;
//} //End GetImageList
	...
	while (nIndex < strStyle.GetLength())
	{
		...
		if (GetIndexNextAlphaNum(strStyle, nIndex))
		{
			...
			if (0 != chSymbol)
			{
				...
				{
					cs.bItaliFont = GetStyleFontStyle(strParameter, cs.bItaliFont);
				}
...
int CPPHtmlDrawer::GetLengthUnit(CPPString & str, int nDefault, BOOL bFont /* = FALSE */)
{
	...
	else if (strUnit == _T("ex"))
	{
		...
		_DC dc;
		dc.Attach(m_hDC);
		dc.GetTextExtentPoint32(&strText, szText);
		dc.Detach();
...
void CPPHtmlDrawer::SetDefaultStyles(_STRUCT_CHANGESTYLE & cs)
{
	...
	m_defStyle.bItaliFont = FALSE;	//Is italic logic font?
	...
	if (!str.IsEmpty())
	{
		...
		for (int i = 0; i < str.GetLength(); i++)
		{
			...
			case _T('b'):
				...
				break;
			case _T('i'):
				m_defStyle.bItaliFont = bSetValue;
				bSetValue = TRUE;
				break;
			case _T('u'):
...
CPPString CPPHtmlDrawer::GetWordWrap(CPPString & str, int nMaxSize, int & nRealSize)
{
	...
	while ((sz.cx <= nRealSize) && (0 != tch))
	{
		...
		s = str.Left(nCurIndex);
		dc.GetTextExtentPoint32(&s, sz);
	...
	else 
	{
		...
		sResult.TrimRight();
		dc.GetTextExtentPoint32(&sResult, sz);
