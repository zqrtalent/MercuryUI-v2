...
BOOL 
CPPToolTip::Create(CWnd* pParentWnd, BOOL bBalloon /* = TRUE */){
...
void 
CPPToolTip::OnActivateApp(BOOL bActive, DWORD hTask){
...
LRESULT 
CPPToolTip::SendNotify(LPPOINT pt, PPTOOLTIP_INFO & ti) { 
//	////TRACE(_T("CPPToolTip::SendNotify()\n")); 
	// Make sure this is a valid window  
	if (!IsWindow(GetSafeHwnd()))  return 0L; 
	// See if the user wants to be notified  
	if (!IsNotify())  return 0L; 
...
BOOL 
CPPToolTip::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) {
	CPoint ptClient;
...
LRESULT 
CPPToolTip::OnRepaintWindow(WPARAM wParam, LPARAM lParam){
...
void 
CPPToolTip::OnDrawBorder(HDC hDC, HRGN hRgn){
	ASSERT (hDC);
	ASSERT (hRgn);
...
////////////////////////////////////////////////////////////////////////
void 
CPPToolTip::OnRedrawTooltip(HDC hDC, BYTE nTransparency /* = 0 */){
...
//	GetWindowRgn(hrgn);
	...
	::BitBlt(hTempDC, m_rcBoundsTooltip.left, m_rcBoundsTooltip.top, m_rcBoundsTooltip.Width(), m_rcBoundsTooltip.Height(), hBkDC, 0, 0, SRCCOPY);
	...
	if (m_szOffsetShadow.cx || m_szOffsetShadow.cy){
		...
		m_drawer.GetDrawManager()->DrawShadow(hTempDC, m_szOffsetShadow.cx, m_szOffsetShadow.cy,
										      rect.Width(), rect.Height(), hMask, m_bGradientShadow, m_szDepthShadow.cx, m_szDepthShadow.cy);
		::DeleteObject(hMask);
		}
	...
	::BitBlt(hDC, m_rcBoundsTooltip.left, m_rcBoundsTooltip.top,
		m_rcBoundsTooltip.Width(), m_rcBoundsTooltip.Height(), hTempDC, 0, 0, SRCCOPY);
...
BOOL 
CPPToolTip::PreTranslateMessage(MSG* pMsg) {
	RelayEvent(pMsg);
	return CWnd::PreTranslateMessage(pMsg);
	}

BOOL 
CPPToolTip::RelayEvent(MSG* pMsg){
...
void 
CPPToolTip::SetNewTooltip(HWND hWnd, const PPTOOLTIP_INFO & ti, BOOL bDisplayWithDelay /* = TRUE */, TooltipType type /* = PPTOOLTIP_NORMAL */){
...
void 
CPPToolTip::OnTimer(UINT nIDEvent) {
	POINT pt;
	switch (nIDEvent)
	{
	case TIMER_SHOW:
...
void 
CPPToolTip::PrepareDisplayTooltip(LPPOINT lpPoint){
...
HWND CPPToolTip::GetWndFromPoint(const LPPOINT lpPoint, PPTOOLTIP_INFO & ti, BOOL bCheckTool /* = TRUE */)
{
	...
	if (NULL != hWnd)
	{
		if (FindTool(hWnd, lpPoint, ti) || !bCheckTool)
			return hWnd;
		/*
...
_String CPPToolTip::GetDebugInfoTool(LPPOINT lpPoint)
{
	PPTOOLTIP_INFO ti;
	...
	if (NULL != hParent)
	{
		...
		str = GetMaxDebugString((_String)ch);
	} //if
	else str = _T("N/A");
...
_String CPPToolTip::CreateDebugCell(_String sTitle, LPCTSTR lpszDescription)
{
	_String str;
	str.Format(_T("<tr><td width=70 bgcolor=buttonface>%s</td><td width=130 bgcolor=window>%s</td></tr>"), 
		sTitle, GetMaxDebugString(lpszDescription).GetBuffer());
	return str;
} //End of CreateDebugCell

_String CPPToolTip::GetMaxDebugString(LPCTSTR lpszText)
{
	_String str = (CString)lpszText;
...
void CPPToolTip::AddTool(CWnd * pWnd, DWORD dwIdString, DWORD dwIdIcon, _Size & szIcon /* = _Size(0, 0) */, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	_String str;
	str.LoadString(dwIdString);
...
void CPPToolTip::AddTool(CWnd * pWnd, LPCTSTR lpszString, DWORD dwIdIcon, _Size & szIcon /* = _Size(0, 0) */, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	_String str;
...
void CPPToolTip::AddTool(CWnd * pWnd, LPCTSTR lpszString, DWORD dwIdBitmap, COLORREF crMask, _Size & szBitmap /*= _Size(0, 0)*/, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	_String str;
...
void CPPToolTip::AddTool(CWnd * pWnd, LPCTSTR lpszString /* = NULL */, LPCRECT lpRectBounds /* = NULL */, DWORD dwIDTool /* = 0 */)
{
	...
	ti.sTooltip = (_String)lpszString;
	ti.nMask = 0;
	ti.nStyles = 0;
	ti.nDirection = 0;
	ti.nEffect = 0;
...
void
CPPToolTip::ActivateToolTip(bool bActivate){
	...
	if( bActivate ){
		...
		if( FindTool(m_hParentWnd, &pt, ti) ){
			ti.nBehaviour = m_dwBehaviour | PPTOOLTIP_DISABLE_AUTOPOP;
			SetNewTooltip(NULL, ti, 1, PPTOOLTIP_NORMAL);
			}
		}
	else{
		HideTooltip();
...
BOOL 
CPPToolTip::GetToolInfo(PPTOOLTIP_INFO & ti, CWnd * pWnd, LPCRECT lpRectBounds /* = NULL */){
	ASSERT(pWnd);
	//ENG: Gets HWND of a window
	HWND hWnd = pWnd->GetSafeHwnd();	
	//ENG: Searching a specified HWND
...
BOOL 
CPPToolTip::GetToolInfo(PPTOOLTIP_INFO & ti, CWnd * pWnd, DWORD dwIDTool /* = 0 */){
	ASSERT(pWnd);
	//ENG: Gets HWND of a window
	HWND hWnd = pWnd->GetSafeHwnd();	
	//ENG: Searching a specified HWND
...
////////////////////////////////////////////////////////////////////
void 
CPPToolTip::SetCallbackHyperlink(HWND hWnd, UINT nMessage, LPARAM lParam /* = 0 */){
...
void CPPToolTip::ShowHelpTooltip(LPPOINT pt, DWORD dwIdText, DWORD dwIdIcon, _Size & szIcon /* = _Size(0, 0) */)
{
	_String str;
	str.LoadString(dwIdText);
...
void CPPToolTip::ShowHelpTooltip(LPPOINT pt, LPCTSTR lpszString, DWORD dwIdIcon, _Size & szIcon /* = _Size(0, 0) */)
{
	_String str;
	str.Format(_T("<table><tr><td><icon idres=%d width=%d height=%d></td><td>%s</td></tr></table>"), 
		dwIdIcon, szIcon.cx, szIcon.cy, lpszString);
