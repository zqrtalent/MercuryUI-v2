...
#define POKER_TOURNAMENT_START_COUNTDOWN	5007
#define POKER_TIMEBANK_COUNTDOWN			5008
...
struct ActionButtonInfo{
	ActionButton	type;
	_String			sLabel;
	INT_CHIPS		nAmountMin;
	INT_CHIPS		nAmount;
	INT_CHIPS		nAmountMax;
	int				nButtonId;
	};
...
public:
	...
	// Static methods.
	static	_String			GetGameName					(int nGameType, int nGameSubType);
	static	_String			GetGameNameEng				(int nGameType, int nGameSubType);
	static  int				GetSubTypesByGameType		(int nGameType);		
	static	_String			GetCurrencyName				(int nCurrencyId);
	static	_String			GetCurrencyNameEng			(int nCurrencyId);
	static	_String			GetFormattedAmount			(int nCurrencyId, INT_CHIPS nAmount);
	static	_String			GetTournamentStatus			(int nStatus);
	static	_String			GetTournamentDateAsString	(DateTime dtTime);
	static	_String			GetTournamentBuyin			(int nBuyinAmount, int nBuyinFeeAmount, int nCurrencyId);
	static	_String			CardNameByIndex				(int nIndex);
};
