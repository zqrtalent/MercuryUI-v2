...
bool
ESChildControlTableCards::Create(int nId, _Rect rcRect, MercuryBaseView* pOwner, ESChildControl* pParent){
...
void
ESChildControlTableCards::OnPaintClient(_DC *pDC, _Rect* pRectDC, _Rect* pRectClient){
	...
	_Rect rcCard;
	rcCard.left		= m_rcRect.left + (m_bCentred ? (m_rcRect.Width() - GetWidthCards(5))/2 : 0) /*Make cards in the middle of control*/;
	...
	while( nArrCards[nLoop] != -1 ){
		...
		rcCardDC.left	+= nCardOffsets[nLoop];
...
ESChildControlTableCards::GetCardRect(int nCardIndex){
	CalcCardRects();
	_Rect* rcArrCards[] = {&m_rcCard1, &m_rcCard2, &m_rcCard3, &m_rcCard4, &m_rcCard5};
...
ESChildControlTableCards::OnMotionProgress(int nFrame, int nCurrTimeMS, int nFrameCt, float fPosX, float fPosY){
	...
	if( m_nCard1 != -1 && m_nCard2 != -1 && m_nCard3 != -1 && m_nCard4 == -1 && m_nCard4Temp == -1 && m_nCard5Temp == -1 ){
		...
		if( (m_nCard4 != -1  || m_nCard4Temp != -1) && m_nCard5 == -1 && m_nCard5Temp == -1 ){
			...
			_Rect							rcRedraw		= pDeck->GetClientRect();
			...
			if( nFrame == 1 ){
				...
				if( nFrame == nFrameCt ){
					rcRectNoChange.SetRect(0, 0, 0, 0);
