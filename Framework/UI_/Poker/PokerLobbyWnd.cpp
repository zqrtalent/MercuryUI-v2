...
bool
PokerLobbyWnd::AttachToWindow(ESFrameBase* pBaseFrameWnd){
...
void
PokerLobbyWnd::SetUserName(_String sUserName, bool bRedraw){
...
void
PokerLobbyWnd::SetUserBalance(INT_CHIPS nBalance, bool bRedraw){
	ESChildControlLabel* pUserBalance = (ESChildControlLabel*)GetChildControlByName(_T("_userBalanceLabel"));
	if( !pUserBalance ) return;

	_String sBalance;
	sBalance.Format(_T("%d.%02d GEL"), (UINT)(nBalance/100), (UINT)(nBalance%100));
	pUserBalance->SetLabel(sBalance, bRedraw);
	}

void
PokerLobbyWnd::SetUserInPlayAmount(INT_CHIPS nInPlayAmount, bool bRedraw){
	...
	_String sBalance;
	sBalance.Format(_T("%d.%02d GEL"), (UINT)(nInPlayAmount/100), (UINT)(nInPlayAmount%100));
...
void
PokerLobbyWnd::SetUserAvatarImage(Image* pImageAvatar, bool bRedraw){
