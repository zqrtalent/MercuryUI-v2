...
class ODBCGridView: public GridView
{
	// < Constructor/Destructor >
public:
	ODBCGridView();
	virtual ~ODBCGridView();

	friend class CODBCHeaderCtrl;

	DECLARE_DYNCREATE(ODBCGridView)
...
public:
	...
	void			DefineColumn		(int nIndex, LPCTSTR lpszTitle, LPCTSTR lpszDBField, int nAlignment, int nWidth, int iImage = -1, LPCTSTR lpszChkField = NULL, int nScaleShow = -1);
...
public:
	...
	static ODBCGridView*	GridViewByName(TCHAR* lpszGridName);
...
__inline CString
ODBCGridView::GetLastError()
	{
	return m_strLastError;
	}

__inline void			
ODBCGridView::SetWaitWhenRowChange(BOOL bWait /*= TRUE*/)
	{
	m_bWaitRowChange = bWait;
	}

__inline void			
ODBCGridView::SetGridOwner(HWND hWndOwner)
	{
	m_hWndOwner = hWndOwner;
	}

__inline int				
ODBCGridView::GetNumberCols()
	{
	return m_nNumberCols;
	}

__inline void	
ODBCGridView::SetAutoRefresh(BOOL bAutoRefresh /*= TRUE*/)
	{
	m_bAutoRefresh = bAutoRefresh;
	}

__inline void
ODBCGridView::SetGenerator(CString sGenName)
	{
	m_sGeneratorName = sGenName;
	}

__inline void
ODBCGridView::SetExtendedStyle(DWORD dwExStyle)
	{
	m_dwExStyle = dwExStyle;
	}

__inline void
ODBCGridView::UseFirstColAsSelState	(BOOL bUse)
	{
	m_bUseFirstColAsSelState = bUse;
	}

__inline void
ODBCGridView::SetSingleSelection(BOOL bSet)
	{
	m_bSingleSel = bSet;
	}

__inline BOOL
ODBCGridView::ColAllowSort(int nCol, BOOL bAllow /*= TRUE*/)
	{
