		...
		if( IsInsertQuery(StatementText, sTable) ){
			...
			while( nFind != -1 )
				...
				if( !IsWindow(hWndGrid) )
					return lRet;
				DataGridView*	pGridView	= (DataGridView*)CWnd::FromHandlePermanent(hWndGrid);
...
//
//	[public] DataGridView::IsCmdHandled
//
BOOL
DataGridView::IsCmdHandled(UINT nCode, UINT nCmdId)
	{
...
//
//	[public] DataGridView::FindKeyMoveTo
//
BOOL
DataGridView::FindKeyMoveTo(LPCTSTR lpszKey, BOOL bMove /*= TRUE*/)
	{
...
//
//	[public] DataGridView::MoveTo
//
void
DataGridView::MoveTo(int nItem)
	{
	SelectItem(nItem);
	}

//
//	[public] DataGridView::GetRowId
//
CString			
DataGridView::GetRowId(int nItem /*= -1*/)
	{
...
//
//	[public] DataGridView::GetRowIdAsLong
//
long
DataGridView::GetRowIdAsLong(int nItem /*= -1*/)
	{
...
//
//	[public] DataGridView::SelectSQLWithUID
//
BOOL			
DataGridView::SelectSQLWithUID(CString& sSelectSQL, CString sUIDField, long lUid)
	{
...
//
//	[public] DataGridView::UIDFromUpdateSQLQuery
//
long			
DataGridView::UIDFromUpdateSQLQuery(CString& sUpdateSQL)
	{
...
//
//	[public] DataGridView::UIDFromDeleteSQLQuery
//
long			
DataGridView::UIDFromDeleteSQLQuery(CString& sDeleteSQL)
	{
...
//
//	[public] DataGridView::RefreshForAddNew
//
void			
DataGridView::RefreshForAddNew()
	{
...
//
//	[public] DataGridView::RefreshForUpdate
//
void
DataGridView::RefreshForUpdate(long lUid)
	{
	ASSERT( m_pRstSelect && !m_pRstSelect->IsOpen() );
	if( !m_bAutoRefresh ) return;
...
//
//	[public] DataGridView::RefreshForDelete
//
void			
DataGridView::RefreshForDelete(long lUid)
	{
	ASSERT( m_pRstSelect && !m_pRstSelect->IsOpen() );
	if( !m_bAutoRefresh ) return;
	m_csection.Lock();
...
//
//	[public] DataGridView::DefineColumn
//
void
DataGridView::DefineColumn(int nIndex, LPCTSTR lpszTitle, LPCTSTR lpszDBField, 
...
//
//	[public] DataGridView::SetRowId
//
void
DataGridView::SetRowId(const CString sUIDField)
	{
	m_sUIDField		= sUIDField;
	}

//
//	[public] DataGridView::SetRst
//
iODBCRst*			
DataGridView::SetRst(iODBCRst* pRst)
	{
	ASSERT( pRst );
...
//
//	[public] DataGridView::LoadOrReload
//
BOOL
DataGridView::LoadOrReload(BOOL bLoad /*= FALSE*/)
	{
	// Is not initialized. #####
	if( m_nNumberCols <= 0 )
		return FALSE;
...
//
//	DataGridView::AdjustBrightness
//
void 
DataGridView::AdjustBrightness(	IN OUT COLORREF&	Color,
...
//
//	[protected] DataGridView::FreeRecordDataArray
//
void
DataGridView::FreeRecordDataArray()
	{
	m_csection.Lock();
...
//
//	[static protected] DataGridView::GetBool_FieldAsText
//
void
DataGridView::GetBool_FieldAsText(ODBCGridCol* pDBCol, CString& sFieldText, CRecord* pRec)
	{
...
//
//	[static protected] DataGridView::GetInt_FieldAsText
//
void		
DataGridView::GetInt_FieldAsText(ODBCGridCol* pDBCol, CString& sFieldText, CRecord* pRec)
	{
	if( pRec->IsFieldNull(pDBCol->nFieldID) )
		{
		sFieldText = _T("0");
...
//
//	[static protected] DataGridView::GetDouble_FieldAsText
//
void
DataGridView::GetDouble_FieldAsText(ODBCGridCol* pDBCol, CString& sFieldText, CRecord* pRec)
	{/*
	if( pRec->IsFieldNull(pDBCol->nFieldID) )
		{
		sFieldText = _T("0");
...
//
//	[static protected] DataGridView::GetDateTime_FieldAsText
//
void
DataGridView::GetDateTime_FieldAsText(ODBCGridCol* pDBCol, CString& sFieldText, CRecord* pRec)
	{
...
//
//	[static protected] DataGridView::GetDate_FieldAsText
//
void		
DataGridView::GetDate_FieldAsText(ODBCGridCol* pDBCol, CString& sFieldText, CRecord* pRec)
	{
	static COleDateTime dtTime;
	if( pRec->IsFieldNull(pDBCol->nFieldID) )
		sFieldText = _T("");
...
//
//	[static protected] DataGridView::GetString_FieldAsText
//
void
DataGridView::GetString_FieldAsText(ODBCGridCol* pDBCol, CString& sFieldText, CRecord* pRec)
	{
	pRec->GetString(pDBCol->nFieldID, sFieldText);
	}

//
//	[static protected] DataGridView::GetDecimal_FieldAsText
//
void		
DataGridView::GetDecimal_FieldAsText(ODBCGridCol* pDBCol, CString& sFieldText, CRecord* pRec)
	{
	pRec->GetString(pDBCol->nFieldID, sFieldText);
	if( sFieldText[0] == '.' )
...
//
//	[protected virtual] DataGridView::GetLastUIDofTable
//
void	
DataGridView::GetLastUIDofTable(long& lUID)
	{
	lUID = 0;
	}
...
//
//	[protected handler] DataGridView::OnShowWindow
//
void	
DataGridView::OnShowWindow(BOOL bShow, UINT nStatus)
	{
	GridView::OnShowWindow(bShow, nStatus);
	}

//
//	[private] DataGridView::ReInitializeHeader
//
void
DataGridView::ReInitializeHeader()
	{
	// No sort column. ###########
	m_nSortCol	= -1;
	m_nSortFlag = SORT_FLAG_NOT;
...
CString 
DataGridView::GetSelectedKeyList( LPCTSTR delimiter )
	{
	CString		keylist;
	BOOL		first		= TRUE;
	int			row			= -1;
...
//
//	[public] DataGridView::GetCheckedKeyList
//

CString
DataGridView::GetCheckedKeyList(LPCTSTR delimiter)
	{
	CString			keylist;
	BOOL			first		= TRUE;
...
//
//	[virtual] DataGridView::OnCheckItem
//
void
DataGridView::OnCheckItem(CRecord* pCurrRecord, int nCol, int nRow, BOOL& bAllow, BOOL bCurrentState)
	{
	bAllow = TRUE;
	}
...
LRESULT	
DataGridView::OnAddRecords(WPARAM wParam, LPARAM lParam)
	{
...
//
//	[afx_msg] DataGridView::OnCheckAll
//
void
DataGridView::OnCheckAll()
	{
	BOOL bRedraw = FALSE;
...
//
//	[afx_msg] DataGridView::OnUnCheckAll
//
void
DataGridView::OnUnCheckAll()
	{
	BOOL bRedraw = FALSE;
...
//
//	[protected] DataGridView::CheckAll
//
void
DataGridView::CheckAll(int nCol, BOOL bCheck, BOOL& bRedraw)
	{
	bRedraw = FALSE;
	if( nCol <= -1 )
		return;
...
//
//	[public] DataGridView::GetCheckBitmaps
//

void
DataGridView::GetCheckBitmaps(HBITMAP& pbmpCheck, HBITMAP& pbmpUnCheck)
	{
	if( pbmpCheck )
...
//
//	[static] DataGridView::CompareDummy
//
int		
DataGridView::CompareDummy(void* lpValue1, void* lpValue2)
	{
	return -1; // Add to last
	}

//
//	[static] DataGridView::CompareTextField
//

int
DataGridView::CompareTextField(void* lpValue1, void* lpValue2)
	{
	return CompareStrings_Geo((LPCTSTR)lpValue1, (LPCTSTR)lpValue2);
	}

//
//	[static] DataGridView::CompareDecimalField
//

int
DataGridView::CompareDecimalField(void* lpValue1, void* lpValue2)
	{
...
//
//	[static] DataGridView::CompareDateField
//
int
DataGridView::CompareDateField(void* lpValue1, void* lpValue2)
	{
	return 0;
	}

//
//	[static] DataGridView::CompareLongField
//
int
DataGridView::CompareLongField(void* lpValue1, void* lpValue2)
	{
	ASSERT( lpValue1 && lpValue2 );
...
//
//	[static] DataGridView::CompareDoubleField
//
int
DataGridView::CompareDoubleField(void* lpValue1, void* lpValue2)
	{
	ASSERT( lpValue1 && lpValue2 );
...
//
//	[static] DataGridView::CompareTextField_Desc
//

int
DataGridView::CompareTextField_Desc(void* lpValue1, void* lpValue2)
	{
	return CompareStrings_Geo((LPCTSTR)lpValue2, (LPCTSTR)lpValue1);
	}

//
//	[static] DataGridView::CompareDecimalField_Desc
//

int
DataGridView::CompareDecimalField_Desc(void* lpValue1, void* lpValue2)
	{
	ASSERT( lpValue1 && lpValue2 );
	double d1 = atof((LPCSTR)lpValue1);
	double d2 = atof((LPCSTR)lpValue2);
...
//
//	[static] DataGridView::CompareDateField_Desc
//
int
DataGridView::CompareDateField_Desc(void* lpValue1, void* lpValue2)
	{
	return 0;
	}

//
//	[static] DataGridView::CompareLongField_Desc
//
int
DataGridView::CompareLongField_Desc(void* lpValue1, void* lpValue2)
	{
	ASSERT( lpValue1 && lpValue2 );
	return (*(long*)lpValue1) > (*(long*)lpValue2) ? -1 : ((*(long*)lpValue1) == (*(long*)lpValue2) ? 0 : 1);
	}
...
//
//	[static] DataGridView::CompareDoubleField_Desc
//
int
DataGridView::CompareDoubleField_Desc(void* lpValue1, void* lpValue2)
	{
	ASSERT( lpValue1 && lpValue2 );
	return (*(double*)lpValue1) > (*(double*)lpValue2) ? -1 : ((*(double*)lpValue1) == (*(double*)lpValue2) ? 0 : 1);
	}
...
//
//	[virtual] DataGridView::SortColumn
//

#include "..\pluginglobal.h"

BOOL
DataGridView::SortColumn(int nCol, BOOL bAsc)
	{
...
//
//	[public] DataGridView::GetCheckedArray
//

BOOL
DataGridView::GetCheckedArray(int nCol, AutoSortedArray& arrIdsChecked, BOOL bChecked /*= TRUE*/)
	{
	CListCtrl&		list	= GetListCtrl();
	CHeaderCtrl*	pHeader	= list.GetHeaderCtrl();
...
//
//	[public] DataGridView::SetMenuId
//

BOOL
DataGridView::SetMenuId(UINT nIDMenu, UINT nPopupIndex /*= 1*/)
	{
	CMenu menu;
...
//
//	[afx_msg] DataGridView::OnContextMenu
//

void
DataGridView::OnContextMenu(CWnd* pWnd, CPoint pos)
	{
...
//
//	[public] DataGridView::EmptyList
//

BOOL
DataGridView::EmptyList()
	{
...
//
//	[public] DataGridView::GetItemText
//

CString
DataGridView::GetItemText(int nRow, int nCol)
	{
	CString sText;
...
//
//	[public] DataGridView::ReloadCols
//

void
DataGridView::ReloadCols()
	{
	ASSERT( m_pRecord );
/*
	CMarkup xmlGrid;
...
//
//	[public] DataGridView::RegSaveGridInfo
//
BOOL
DataGridView::RegSaveGridInfo()
	{
	...
	if( !pApp->WriteProfileStringA("DataGridView", m_sGridName.GetBuffer(), sGridInfoXML) )
		return FALSE;
	return TRUE;
	}

//
//	[public] DataGridView::RegLoadGridInfo
//

ODBCGridCol* 
DataGridView::RegLoadGridInfo(long& lColCount)
	{
	...
	CString sGridInfoXML = pApp->GetProfileString("DataGridView", m_sGridName.GetBuffer(), "");
...
//
//	[protected] DataGridView::BuildODBCColumnsArray
//
void
DataGridView::BuildODBCColumnsArray()
	{
	if( m_nNumberCols <= 0 )
		return;
...
//
//	[protected] DataGridView::GetODBCGridCol
//
BOOL
DataGridView::GetODBCGridCol(ODBCGridCol* pCol)
	{
	if( !m_pRecord )
		return FALSE;
...
// ===============================================================================
DataGridView*
DataGridView::GridViewByName(TCHAR* lpszGridName)
	{
	return (DataGridView*)GlobalStorage::GetInstance()->GetNamedObject(lpszGridName);
	}
...
CString
DataGridView::GetString(TCHAR* lpszGridName, TCHAR* lpszField, int nRow /*= -1*/)
	{
	ASSERT( nRow >= -1 );
	DataGridView* pGrid = GridViewByName(lpszGridName);
	if( pGrid ){
		// Lock
...
double
DataGridView::GetDouble(TCHAR* lpszGridName, TCHAR* lpszField, int nRow /*= -1*/, double dDefault /*= 0.0*/)
	{
	ASSERT( nRow >= -1 );
	DataGridView* pGrid = GridViewByName(lpszGridName);
	if( pGrid ){
...
long
DataGridView::GetLong(TCHAR* lpszGridName, TCHAR* lpszField, int nRow /*= -1*/, long lDefault /*= 0*/)
	{
	ASSERT( nRow >= -1 );
	DataGridView* pGrid = GridViewByName(lpszGridName);
	if( pGrid ){
...
BOOL
DataGridView::GetBool(TCHAR* lpszGridName, TCHAR* lpszField, int nRow /*= -1*/, BOOL bDefault /*= FALSE*/)
	{
	ASSERT( nRow >= -1 );
	DataGridView* pGrid = GridViewByName(lpszGridName);
	if( pGrid ){
...
BOOL
DataGridView::GetDate(TCHAR* lpszGridName, TCHAR* lpszField, COleDateTime& dtValue, int nRow /*= -1*/)
	{
	ASSERT( nRow >= -1 );
	DataGridView* pGrid = GridViewByName(lpszGridName);
	if( pGrid ){
...
int
DataGridView::GetRowCount(TCHAR* lpszGridName)
	{
	DataGridView* pGrid = GridViewByName(lpszGridName);
	if( pGrid ){
...
CString
DataGridView::GetField(TCHAR* lpszGridName, int nField)
	{
	DataGridView* pGrid = GridViewByName(lpszGridName);
...
int
DataGridView::GetFieldCt(TCHAR* lpszGridName)
	{
	DataGridView* pGrid = GridViewByName(lpszGridName);
	if( pGrid && pGrid->m_pRecord ){
