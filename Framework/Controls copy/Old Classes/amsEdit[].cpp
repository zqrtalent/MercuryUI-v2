...
/*
	if( nStart < nEnd )
		{
		// ZqrTalent [6.9.2009]
		// Transform copied geo nonunicode text to UNICODE text. ######
		if( ::OpenClipboard(m_hWnd) )
			...
			if( ::IsClipboardFormatAvailable(CF_UNICODETEXT) )
				...
				if( hClipBoardData )
					{
					wchar_t*	lpswUnicodeText	= (LPWSTR)hClipBoardData;
					wchar_t*	lpwOut			= NULL;
					CString		sAnsi;
					int			nLoop			= 0;
					while( lpswUnicodeText[nLoop] != 0x0000 )
						{
						sAnsi += (char)lpswUnicodeText[nLoop];
						nLoop ++;
						}
					
					::PluginGlobal::GeoAnsiToUnicode(sAnsi, lpwOut);
					::EmptyClipboard();

					HGLOBAL hglbCopy = GlobalAlloc(GMEM_MOVEABLE, (sAnsi.GetLength() + 1)*sizeof(wchar_t)); 
					if( hglbCopy == NULL ) 
						{ 
						CloseClipboard(); 
						return 0; 
					...
					memcpy(lptstrCopy, lpwOut, (sAnsi.GetLength() + 1)*sizeof(wchar_t));
					delete lpwOut; 
...
/*
	// ZqrTalent [6.9.2009]
	// Transform copied UNICODE text Georgian nonunicode text. ######
	if( ::OpenClipboard(m_hWnd) )
		{
		if( ::IsClipboardFormatAvailable(CF_UNICODETEXT) )
			{
			...
			if( !hClipBoardData )
				{
		//		hClipBoardData = GetClipboardDataAlloc(CF_UNICODETEXT);
		//		if( hClipBoardData )
		//			bFreeLocal = TRUE;
				}
			...
			if( hClipBoardData )
				{
				wchar_t* lpswUnicodeText	= (LPWSTR)hClipBoardData;
				::PluginGlobal::UnicodeToGeoAnsi(lpswUnicodeText, sClipboardText);
				bDoDefault					= FALSE;

				int		nCurSel = sClipboardText.GetLength() - 1;
				CString sLeft, sRight;
				
				CString sText;
				GetWindowText(sText);
				sLeft			= sText.Left	(nStart);
				sRight			= sText.Right	(sText.GetLength() - nEnd);

				sText			= sClipboardText;
				sClipboardText	= sLeft + sText + sRight;
				nCurSel			= sLeft.GetLength() + sText.GetLength();
				
				SendMessage	(WM_SETTEXT, 0L, (LPARAM)sClipboardText.GetBuffer());
				SetSel		(nCurSel, nCurSel);
...
void 
CAMSEdit::Behavior::_OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
	{
	// I want to use nonuicode symbols with english.
	// System converts typed symbols and this is the problem.
	// Sulution of the problem is written below.
	// ZqrTalent 28/09/2007
	CString strText;
	...
	int nTextLen = m_pEdit->GetWindowTextLength();
	if( nTextLen - (nStart - nEnd) == strText.GetLength() + nRepCnt )
		{
		strText.Delete(nStart, abs(nEnd - nStart));
		while( nRepCnt-- )
			strText.Insert(nStart, (BYTE)uChar);
		m_pEdit->SetWindowText(strText);
		m_pEdit->SetSel(nStart+1, nStart+1);
		}
	}
...
CString CAMSEdit::NumericBehavior::_GetValidText() const
{
	...
	int iPos = 0, nDecimalPos = -1, nNewLen = 0, nLen;
