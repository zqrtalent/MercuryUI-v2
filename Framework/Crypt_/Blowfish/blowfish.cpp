...
void 
CBlowFish::Blowfish_encipher (DWORD *xl, DWORD *xr){
	union aword  Xl, Xr ;
	Xl.dword = *xl ;
	Xr.dword = *xr ;
...
void 
CBlowFish::Blowfish_decipher (DWORD *xl, DWORD *xr){
   union aword  Xl ;
   union aword  Xr ;
...
DWORD 
CBlowFish::GetOutputLength (DWORD lInputLong){
	DWORD 	lVal ;
...
DWORD 
CBlowFish::Encode (BYTE * pInput, BYTE * pOutput, DWORD lSize){
...
// output buffer can be the same, but be sure buffer length is even MOD 8.
void 
CBlowFish::Decode (BYTE * pInput, BYTE * pOutput, DWORD lSize){
	DWORD 	lCount ;
	BYTE	*pi, *po ;
	int		i ;
	int		SameDest = (pInput == pOutput ? 1 : 0) ;
