...
public:
	...
	void		DrawRectangle				(RECTDef* pRect, int nBorderWidth, COLORREF crFill, COLORREF crBorder);
	void		DrawRectangleBorder			(RECTDef* pRect, int nBorderWidth, COLORREF crBorder);
	void		DrawLine					(POINTDef* pLineFrom, POINTDef* pLineTo, int nLineWidth, COLORREF crLine);

	BOOL		ExtTextOut					(int x, int y, UINT options, LPCRECTDef lprect, _String& text, const int * lpDx = NULL);
