...
// Operations
public:
	int		Display(void** lpRetrieveDialogTemplate = NULL);
...
int XMessageBox(HWND hwnd,
				...
				void** lpRetrieveDialogTemplate /*= NULL*/
				)
{
	...
	int rc = dlg.Display(lpRetrieveDialogTemplate);
	return rc;
}
	...
	m_bRightJustifyButtons(pXMB->dwOptions & XMSGBOXPARAMS::RightJustifyButtons)
{
	...
	strcpy(ncm.lfMessageFont.lfFaceName,"GeoMegasoft");
	...
	m_dlgTempl.style = WS_CAPTION | WS_VISIBLE | WS_SYSMENU |
		WS_POPUP | DS_MODALFRAME | DS_CENTER | DS_SETFOREGROUND;
		...
		case WM_INITDIALOG:
		{
			...
			for (nID = 1; nID <= ID_XMESSAGEBOX_LAST_ID; nID++)
			{
				...
				if (hwndChild && ::IsWindow(hwndChild))
				{
					...
					if(nID==IDOK)
					{
						SetWindowText(hwndChild,"OK");
					}
					else if(nID==IDCANCEL)
					{
						SetWindowText(hwndChild,"ïôûìäô");
					}
					else if(nID==IDABORT)
					{
						SetWindowText(hwndChild,"ÛóÑóêóèô");
					}
					else if(nID==IDIGNORE)
					{
						SetWindowText(hwndChild,"øïòùêøêóèô");
					}
					else if(nID==IDNO)
					{
						SetWindowText(hwndChild,"ôêô");
					}
					else if(nID==IDYES)
					{
						SetWindowText(hwndChild,"âøô÷");
					}
					else if(nID==IDRETRY)
					{
						SetWindowText(hwndChild,"Ûóóñôâó");
					}
			...
			else if (GetParent(hwnd) != NULL)
			{
				...
				if( mainRect.left < 0 || mainRect.right < 0 || 
					mainRect.top < 0 || mainRect.bottom < 0 )
					{
		...
		case WM_COMMAND:
		{
			...
			if (Me->GetDefaultButtonId())
			{
				...
				if (hwndDefButton && ::IsWindow(hwndDefButton))
				{
					...
					{
						::SetWindowText(hwndDefButton, "ëôêïø");
					}
				}
			}
...
int CXDialogTemplate::Display(void** lpRetrieveDialogTemplate /*= NULL*/)
{
	...
	LPCDLGTEMPLATE	lpTemplate		= (LPCDLGTEMPLATE)new BYTE[4096];
	BYTE*			pTemplateBuf	= (BYTE*)lpTemplate;
	int				nOffset			= 0;


	((WORD*)&pTemplateBuf[nOffset])[0] = 0x0001;// dialog version.
	nOffset				+= sizeof(WORD);
	((WORD*)&pTemplateBuf[nOffset])[0] = 0xffff; // dialog template type.
	nOffset				+= sizeof(WORD);
	((DWORD*)&pTemplateBuf[nOffset])[0] = m_nHelpId; // help identifier not supported.
	nOffset				+= sizeof(DWORD);
	((DWORD*)&pTemplateBuf[nOffset])[0] = m_dlgTempl.dwExtendedStyle; // extended style.
	nOffset				+= sizeof(DWORD);
	((DWORD*)&pTemplateBuf[nOffset])[0] = m_dlgTempl.style; // style.
	nOffset				+= sizeof(DWORD);
	((WORD*)&pTemplateBuf[nOffset])[0] = m_dlgTempl.cdit; // item count.
	nOffset				+= sizeof(WORD);
	((short*)&pTemplateBuf[nOffset])[0] = m_dlgTempl.x; // left position.
	nOffset				+= sizeof(short);
	((short*)&pTemplateBuf[nOffset])[0] = m_dlgTempl.y; // top position.
	nOffset				+= sizeof(short);
	((short*)&pTemplateBuf[nOffset])[0] = m_dlgTempl.cx; // dialog width.
	nOffset				+= sizeof(short);
	((short*)&pTemplateBuf[nOffset])[0] = m_dlgTempl.cy; // dialog height.
	nOffset				+= sizeof(short);
	((WORD*)&pTemplateBuf[nOffset])[0] = 0;// menu existance. (No menu)
	nOffset				+= sizeof(WORD);
	((WORD*)&pTemplateBuf[nOffset])[0] = (WORD)0; // Dialog class existance
	nOffset				+= sizeof(WORD);
	...
	for (i = 0; i < m_dlgTempl.cdit; i++)
	{
		...
		int nChars = (int)_tcslen(m_pDlgItemArray[i]->m_szCaption) + 1;
	...
	*(WORD*)(pdest + 1) = 0; // use default window class
	...
	memcpy(&pTemplateBuf[nOffset], pchCaption, nActualChars * sizeof(WCHAR)); // Caption text
	nOffset				+= nActualChars * sizeof(WCHAR);

	pdest += nActualChars * sizeof(WCHAR);
	delete pchCaption;

	((WORD*)&pTemplateBuf[nOffset])[0] = 8;	// font height.
	nOffset		+= sizeof(WORD);
	((WORD*)&pTemplateBuf[nOffset])[0] = FW_NORMAL; // font weight.
	nOffset		+= sizeof(WORD);
	((BYTE*)&pTemplateBuf[nOffset])[0] = 0;		// font italic.
	nOffset		+= sizeof(BYTE);
	((BYTE*)&pTemplateBuf[nOffset])[0] = ANSI_CHARSET;	// font charset.
	nOffset		+= sizeof(BYTE);

	wchar_t* lpwFontFaceName = L"Arial";
	int nLen = (int)wcslen(lpwFontFaceName);
	memcpy(&pTemplateBuf[nOffset], lpwFontFaceName, (nLen + 1)*sizeof(WORD));
	nOffset		+= (nLen + 1)*sizeof(WORD); // font face name.
...
#endif
		// Make alignement. {{
		if( nOffset%sizeof(DWORD) )
			nOffset += nOffset%sizeof(DWORD);
		// }}

		((DWORD*)&pTemplateBuf[nOffset])[0] = 1; // help Id.
		nOffset		+= sizeof(DWORD);
		((DWORD*)&pTemplateBuf[nOffset])[0] = m_pDlgItemArray[i]->m_dlgItemTemplate.dwExtendedStyle; // Extended style.
		nOffset		+= sizeof(DWORD);
		((DWORD*)&pTemplateBuf[nOffset])[0] = m_pDlgItemArray[i]->m_dlgItemTemplate.style; // Style.
		nOffset		+= sizeof(DWORD);
		((short*)&pTemplateBuf[nOffset])[0] = m_pDlgItemArray[i]->m_dlgItemTemplate.x; // left position.
		nOffset		+= sizeof(short);
		((short*)&pTemplateBuf[nOffset])[0] = m_pDlgItemArray[i]->m_dlgItemTemplate.y; // top position.
		nOffset		+= sizeof(short);
		((short*)&pTemplateBuf[nOffset])[0] = m_pDlgItemArray[i]->m_dlgItemTemplate.cx; // width.
		nOffset		+= sizeof(short);
		((short*)&pTemplateBuf[nOffset])[0] = m_pDlgItemArray[i]->m_dlgItemTemplate.cy; // height.
		nOffset		+= sizeof(short);
		((DWORD*)&pTemplateBuf[nOffset])[0] = m_pDlgItemArray[i]->m_dlgItemTemplate.id; // Control Identifier.
		nOffset		+= sizeof(DWORD);
		((WORD*)&pTemplateBuf[nOffset])[0] = (WORD)-1; // Window class existance
		nOffset	+= sizeof(WORD);
		((WORD*)&pTemplateBuf[nOffset])[0] = m_pDlgItemArray[i]->m_controltype; // window class identifier.
		nOffset		+= sizeof(WORD);

		_ASSERTE(nActualChars > 0);
		memcpy(pdest, pchCaption, nActualChars * sizeof(WCHAR));
		memcpy(&pTemplateBuf[nOffset], pchCaption, nActualChars * sizeof(WCHAR)); // Item text.
		nOffset += (nActualChars * sizeof(WCHAR));
		pdest += nActualChars * sizeof(WCHAR);
		delete pchCaption;

		WORD wExtraCt = 0;
		((WORD*)&pTemplateBuf[nOffset])[0] = wExtraCt; // Extra byte count.
		nOffset	+= (wExtraCt + 1)*sizeof(WORD);
	...
	if( lpRetrieveDialogTemplate != NULL ){
		*lpRetrieveDialogTemplate = new BYTE[nOffset];
		memcpy(*lpRetrieveDialogTemplate, pTemplateBuf, nOffset);
