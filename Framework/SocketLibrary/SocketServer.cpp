...
"-----END PRIVATE KEY-----\n";
	*/
	...
	InitializeCriticalSection(&m_lockListOfSockets);
	} 
...
bool
SocketServer::Start(SockListenOptions* pInfo, SocketServerEventHandler* pHandler, LPCTSTR lpszPemFile /*= NULL*/, SocketProtocolBase* pProtocol /*= NULL*/){
	...
	if( lpszPemFile ){
		BYTE*	lpData		= NULL;
		int		nFileSize	= 0;
		_String sPemFile	= lpszPemFile;
		Path::BuildFullPath(sPemFile);

		if( Path::ReadFileContent(sPemFile.GetBuffer(), lpData, nFileSize) ){
...
SocketServer::Stop(){
	...
	EnterCriticalSection(&m_lockListOfSockets); // Lock
	...
	LeaveCriticalSection(&m_lockListOfSockets); // Unlock
	return true;
	}
...
void
SocketServer::OnAcceptConnection(SOCKET	sock, const CString sRemoteIp, int nRemotePort, bool& bAccept){
	// Call event handler.
	if( m_pHandler ){
	...
	if( m_options.m_nPort == 843 ){
		...
		if( dwSize == nLenPolicyReq ){
			...
			if( !_strcmpi((const char*)pData, pcszPlicyReq) ){
				...
				pConn->Send((BYTE*)m_sPolicyXML.GetBuffer(), m_sPolicyXML.GetLength() + 1);
...
bool
SocketServer::GetIpAddress(SOCKET sock, CString& sIpLocal, int& nPortLocal, CString& sIpRemote, int& nPortRemote,  DWORD* pIpRemote){
	...
	sIpLocal.Format(_T("%d.%d.%d.%d"), arrIpLocal[0], arrIpLocal[1], arrIpLocal[2], arrIpLocal[3]);
	sIpRemote.Format(_T("%d.%d.%d.%d"), arrIpRemote[0], arrIpRemote[1], arrIpRemote[2], arrIpRemote[3]);
