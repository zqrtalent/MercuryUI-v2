...
SocketContext::SocketContext(UINT nSendBuffSize /*= 8192*/, UINT nReceiveBuffSize /*= 8192*/) :
	...
	::InitializeCriticalSection		(&m_lockSend);
	} 
...
SocketContext::~SocketContext(){
	...
	::DeleteCriticalSection(&m_lockSend);
	}

bool
SocketContext::Connect(TCHAR* pszIp, int nPort, bool bKeepAlive /*= false*/, int nKeepAliveTime /*= 0*/, int nKeepAliveInterval /*= 0*/, SOCKET* pCopySocket /*= NULL*/, HANDLE hConnectEvent /*= NULL*/){
...
bool
SocketContext::ConnectSync(TCHAR* pszIp, int nPort, bool bKeepAlive, int nKeepAliveTime, int nKeepAliveInterval, SOCKET* pCopySocket){
	return false;
	}
...
SocketContext::GetLocalIp(char* pszIp, int nLen, DWORD* pIpAsDWORD /*= NULL*/){
	...
	return true;
	//return (WSAAddressToStringA((sockaddr*)&m_addresses[0], sizeof(sockaddr_storage), NULL, pszIp, (DWORD*)&nLen) != SOCKET_ERROR);
	}
...
SocketContext::GetRemoteIp(char* pszIp, int nLen, DWORD* pIpAsDWORD /*= NULL*/){
	...
	if( pIpAsDWORD )
		*pIpAsDWORD = ((((DWORD)arrIp[0]) << 24) & 0xFF000000) | ((((DWORD)arrIp[1]) << 16) & 0x00FF0000) | ((((DWORD)arrIp[2]) << 8) & 0x0000FF00) | (((DWORD)arrIp[3]) & 0x000000FF);
	return true;
	//return (WSAAddressToStringA((sockaddr*)&m_addresses[1], sizeof(sockaddr_storage), NULL, pszIp, (DWORD*)&nLen) != SOCKET_ERROR);
	}
...
SocketContext::SendBytes(){
	...
	::EnterCriticalSection(&m_lockSend); // Lock
	BYTE*	lpFullData		= NULL;
	BYTE*	lpData			= PopSendData(nSize, bIsPartial, bIsSSLEncrypted, lpFullData);
	if( lpData )
		m_bSending			= true;
	::LeaveCriticalSection(&m_lockSend); // Unlock
	if( !lpData ) return false;

	bool	bDestroyBuffer	= false;
	bool	bRet			= true;
	BYTE*	lpDataSendBuff	= (BYTE*)m_buffSend.buf;
	DWORD	dwSizeSendBuff	= m_nSendBufferSize;
...
//	m_status				|= SockStatus_Sending;
	if( m_pSSL && !bIsSSLEncrypted ){
		...
		// Pass send data to protocol.
		ProtocolOpResultCode	result	= ProtocolOpResultCode::Protocol_Success;
		if( m_pProtocol ){
			result	= m_pProtocol->OnSerializeData(this, lpData, (DWORD)nSize, m_buffSend, dwSizeSendBuff);
			bRet	= (result == ProtocolOpResultCode::Protocol_Success);
			}
		else{
...
SocketContext::ReceiveBytes(){
	if( (m_status&(SockStatus_Connected|SockStatus_Handshaking)) == 0 ){
		/*Temp*/
		//Logger::CreateLogEntry(_T("ReceiveBytes : not connected ..."));
		/*Temp*/
		return false; // Socket is not connected.
		}
	if( m_bReceiving ){
		//Logger::CreateLogEntry(_T("ReceiveBytes : already receiving ..."));
	...
	/*Temp*/
//	_String s;
//	s.Format(_T("ReceiveBytes : set receiving flag ...  bytes received %d"), m_nIOWorkersReferenceCounter);
	//Logger::CreateLogEntry(s);
	//Logger::CreateLogEntry(_T("ReceiveBytes : set receiving flag ..."));
	/*Temp*/
...
bool
SocketContext::ReadReceivedBytes(DWORD dwSize, bool& bCloseSocket){
	/* Dump received bytes.
	TRACE("\r\nreceived bytes size: %d\r\n", dwSize);
	for(int i=0; i<dwSize; i++){
		BYTE bt = m_buffRecv.buf[i];
		TRACE("%02X ", bt);
		if( ((i+1)%16) == 0 ) 
			TRACE("\r\n");
		}*/
	...
	ProtocolOpResultCode	result			= ProtocolOpResultCode::Protocol_Success;
	BYTE*					lpDataReceived	= (BYTE*)m_buffRecv.buf;
	DWORD					dwDataSize		= dwSize;
	bool					bDestroyBuffer	= false;
	bool					bRet			= true;
	if( dwSize > 0 && m_pProtocol ){
		result	= m_pProtocol->OnDeserializeData(this, (BYTE*)m_buffRecv.buf, dwSize, m_nSendBufferSize, lpDataReceived, dwDataSize, bDestroyBuffer);
		bRet			= (result == ProtocolOpResultCode::Protocol_Success);
		bCloseSocket	= (result == ProtocolOpResultCode::Protocol_ConnClose || result == ProtocolOpResultCode::Protocol_Invalid);
		}

	if( lpDataReceived ){
		if( bRet ){
			m_dwBytesReceived += dwDataSize;
			m_receivedData.AddBytes(lpDataReceived, dwDataSize);
			}
		if( bDestroyBuffer )
			delete [] lpDataReceived;
		}
	return bRet;
	}
...
SocketContext::PushSendData(BYTE* lpData, int nDataSize, bool bSSLEncrypted){
	...
	bool bRet = true;
	if( !m_bSending ){
		//TRACE("PushSendData send bytes %d ......\r\n", nDataSize);
		bRet = SendBytes();
		}
	return true;
	}
...
BYTE*
SocketContext::PopSendData(int& nDataSize, bool& bIsPartial, bool& bSSLEncrypted, BYTE*& lpFullData){
	if( !m_listSendData.GetCount() )
		return NULL;

	BYTE*			pResult;
	SendDataBuffer* pBuff	= m_listSendData.GetAt(0);
	lpFullData				= pBuff->_pszBuffer;
...
//
void
SocketContext::OnHandshakeComplete(){
	::EnterCriticalSection(&m_lock); // Lock
	m_status &= ~(SockStatus_Accepting|SockStatus_Connecting|SockStatus_Handshaking);
...
SocketContext::OnAccept(){
	...
	if( m_pProtocol ){
		bool bHandshaking = false;
		m_pProtocol->OnAccepting(this, bHandshaking);
		if( bHandshaking ){
...
SocketContext::OnDataReceived(DWORD dwSize){
	//Logger::CreateLogEntry(_T("OnDataReceived : remove receiving flag ..."));
	::EnterCriticalSection(&m_lock); // Lock
	...
	if( m_pSSL ){
		...
		int		ssl_error			= 0;
		if( ReadReceivedBytes(dwSize, bCloseSocket) ){
		...
		if( bFatalErrorOccurred || bCloseSocket ){
			::LeaveCriticalSection(&m_lock); // Unlock
			Close(m_hEventClosed);
			return;
			}
...
SocketContext::OnDataSent(DWORD dwSize){
	...
	// Error correction: Causes problems when we use Send method in cycle.
	//m_status		&= ~SockStatus_Sending;
	DecreaseIOWorkersReferenceCounter();
...
SocketContext*
SocketContext::BeginAccept(	SocketConnectionListener* pConnListener, SocketServer* pServer, SOCKET sockListen, SSL_CTX* pSSLContext, int nSendBuffSize, int nReceiveBuffSize, bool bKeepAlive /*=false*/, int nKeepAliveTime /*=0*/, int nKeepAliveInterval /*=0*/ ){
	if( sockListen == INVALID_SOCKET )
		return NULL;

	SocketContext* pSockContxt			= new SocketContext(nSendBuffSize, nReceiveBuffSize);
	pSockContxt->m_pProtocol			= pServer->GetProtocol();
