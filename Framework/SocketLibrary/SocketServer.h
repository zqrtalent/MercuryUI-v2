...
public:
	virtual bool					Start				(SockListenOptions* pInfo, SocketServerEventHandler* pHandler, LPCTSTR lpszPemFile = NULL, SocketProtocolBase* pProtocol = NULL);
	virtual bool					Stop				();

	void							SetPolicyXML		(const CString sPolicyXML){m_sPolicyXML = sPolicyXML;};
	int								GetConnectionCt		(){ return m_nConnectionCt; };
	ConnectedSocketInfo*			GetConnectionInfos	(int& nCt);
	static bool						GetIpAddress		(SOCKET sock, CString& sIpLocal, int& nPortLocal, CString& sIpRemote, int& nPortRemote, DWORD* pIpRemote = NULL);
...
protected:
	...
	SocketProtocolBase*					GetProtocol					() {return m_pProtocol;};

protected: // Events
	virtual void						OnAcceptConnection			(SOCKET sock, const CString sRemoteIp, int nRemotePort, bool& bAccept);
